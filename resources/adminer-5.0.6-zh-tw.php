...
sprintf('%.3f ç§’',max(0,microtime(true)-$_h));}function
relative_uri(){return
...
upload_error($m){$Fe=($m==UPLOAD_ERR_INI_SIZE?ini_get("upload_max_filesize"):0);return($m?'ç„¡æ³•ä¸Šå‚³æª”æ¡ˆã€‚'.($Fe?" ".sprintf('å…è¨±çš„æª”æ¡ˆä¸Šé™å¤§å°ç‚º %sBã€‚',$Fe):""):'æª”æ¡ˆä¸å­˜åœ¨ã€‚');}function
...
script("$Zg.onclick = function () { return confirm('".($Je?js_escape($Je):'ä½ ç¢ºå®šå—ï¼Ÿ')."'); };","");}function
...
query($G,$yi=false){$H=$this->pdo->query($G);$this->error="";if(!$H){list(,$this->errno,$this->error)=$this->pdo->errorInfo();if(!$this->error)$this->error='æœªçŸ¥éŒ¯èª¤ã€‚';return
...
connect($Ab){list(,,$E)=$Ab;if($E!="")return'è³‡æ–™åº«ä¸æ”¯æ´å¯†ç¢¼ã€‚';return
new
Db;}function
...
h($f->error);}function
check_sqlite_name($B){global$f;$Ic="db|sdb|sqlite";if(!preg_match("~^[^\\0]*\\.($Ic)\$~",$B)){$f->error=sprintf('è«‹ä½¿ç”¨ä¸‹åˆ—å…¶ä¸­ä¸€å€‹æ“´å……æ¨¡çµ„ %sã€‚',str_replace("|",", ",$Ic));return
false;}return
true;}function
create_database($j,$fb){global$f;if(file_exists($j)){$f->error='æª”æ¡ˆå·²å­˜åœ¨ã€‚';return
...
enumLength($n){$sc=$this->types['ä½¿ç”¨è€…é¡å‹'][$n["type"]];return($sc?type_values($sc):"");}function
setUserTypes($xi){$this->types['ä½¿ç”¨è€…é¡å‹']=array_flip($xi);}function
...
selectLinks($Jh,$N=""){global$l;echo'<p class="links">';$te=array("select"=>'é¸æ“‡è³‡æ–™');if(support("table")||support("indexes"))$te["table"]='é¡¯ç¤ºçµæ§‹';$Zd=false;if(support("table")){$Zd=is_view($Jh);if($Zd)$te["view"]='ä¿®æ”¹æª¢è¦–è¡¨';else$te["create"]='ä¿®æ”¹è³‡æ–™è¡¨';}if($N!==null)$te["edit"]='æ–°å¢é …ç›®';$B=$Jh["Name"];foreach($te
...
selectQuery($G,$_h,$Lc=false){global$l;$I="</p>\n";if(!$Lc&&($Yi=$l->warnings())){$u="warnings";$I=", <a href='#$u'>".'è­¦å‘Š'."</a>".script("qsl('a').onclick = partial(toggle, '$u');","")."$I<div id='$u' class='hidden'>\n$Yi</div>\n";}return"<p><code class='jush-".JUSH."'>".h(str_replace("\n"," ",$G))."</code> <span class='time'>(".format_time($_h).")</span>".(support("sql")?" <a href='".h(ME)."sql=".urlencode($G)."'>".'ç·¨è¼¯'."</a>":"").$I;}function
...
as$n){echo"<tr><th>".h($n["field"]);$U=h($n["full_type"]);echo"<td><span title='".h($n["collation"])."'>".(in_array($U,(array)$Ch['ä½¿ç”¨è€…é¡å‹'])?"<a href='".h(ME.'type='.urlencode($U))."'>$U</a>":$U)."</span>",($n["null"]?" <i>NULL</i>":""),($n["auto_increment"]?" <i>".'è‡ªå‹•éå¢'."</i>":"");$k=h($n["default"]);echo(isset($n["default"])?" <span title='".'é è¨­å€¼'."'>[<b>".($n["generated"]?"<code class='jush-".JUSH."'>$k</code>":$k)."</b>]</span>":""),(support("comment")?"<td>".h($n["comment"]):""),"\n";}echo"</table>\n","</div>\n";}function
...
as$t=>$w){if($w["type"]=="FULLTEXT")echo"<div>(<i>".implode("</i>, <i>",array_map('Adminer\h',$w["columns"]))."</i>) AGAINST"," <input type='search' name='fulltext[$t]' value='".h($_GET["fulltext"][$t])."'>",script("qsl('input').oninput = selectFieldChange;",""),checkbox("boolean[$t]",1,isset($_GET["boolean"][$t]),"BOOL"),"</div>\n";}$Ra="this.parentNode.firstChild.onchange();";foreach(array_merge((array)$_GET["where"],array(array()))as$t=>$X){if(!$X||("$X[col]$X[val]"!=""&&in_array($X["op"],$this->operators)))echo"<div>".select_input(" name='where[$t][col]'",$e,$X["col"],($X?"selectFieldChange":"selectAddRow"),"(".'ä»»æ„ä½ç½®'.")"),html_select("where[$t][op]",$this->operators,$X["op"],$Ra),"<input type='search' name='where[$t][val]' value='".h($X["val"])."'>",script("mixin(qsl('input'), {oninput: function () { $Ra }, onkeydown: selectSearchKeydown, onsearch: selectSearchSearch});",""),"</div>\n";}echo"</div></fieldset>\n";}function
selectOrderPrint($xf,$e,$x){print_fieldset("sort",'æ’åº',$xf);$t=0;foreach((array)$_GET["order"]as$y=>$X){if($X!=""){echo"<div>".select_input(" name='order[$t]'",$e,$X,"selectFieldChange"),checkbox("desc[$t]",1,isset($_GET["desc"][$y]),'é™å†ª (éæ¸›)')."</div>\n";$t++;}}echo"<div>".select_input(" name='order[$t]'",$e,"","selectAddRow"),checkbox("desc[$t]",1,false,'é™å†ª (éæ¸›)')."</div>\n","</div></fieldset>\n";}function
selectLimitPrint($z){echo"<fieldset><legend>".'é™å®š'."</legend><div>","<input type='number' name='limit' class='size' value='".h($z)."'>",script("qsl('input').oninput = selectFieldChange;",""),"</div></fieldset>\n";}function
selectLengthPrint($Zh){if($Zh!==null)echo"<fieldset><legend>".'Text é•·åº¦'."</legend><div>","<input type='number' name='text_length' class='size' value='".h($Zh)."'>","</div></fieldset>\n";}function
...
messageQuery($G,$ai,$Lc=false){global$l;restart_session();$zd=&get_session("queries");if(!$zd[$_GET["db"]])$zd[$_GET["db"]]=array();if(strlen($G)>1e6)$G=preg_replace('~[\x80-\xFF]+$~','',substr($G,0,1e6))."\nâ€¦";$zd[$_GET["db"]][]=array($G,time(),$ai);$wh="sql-".count($zd[$_GET["db"]]);$I="<a href='#$wh' class='toggle'>".'SQL å‘½ä»¤'."</a>\n";if(!$Lc&&($Yi=$l->warnings())){$u="warnings-".count($zd[$_GET["db"]]);$I="<a href='#$u' class='toggle'>".'è­¦å‘Š'."</a>, $I<div id='$u' class='hidden'>\n$Yi</div>\n";}return" <span class='time'>".@date("H:i:s")."</span>"." $I<div id='$wh' class='hidden'><pre><code class='jush-".JUSH."'>".shorten_utf8($G,1000)."</code></pre>".($ai?" <span class='time'>($ai)</span>":'').(support("sql")?'<p><a href="'.h(str_replace("db=".urlencode(DB),"db=".urlencode($_GET["db"]),ME).'sql=&history='.(count($zd[$_GET["db"]])-1)).'">'.'ç·¨è¼¯'.'</a>':'').'</div>';}function
...
as$Xf=>$X){if(!$Xf||preg_match("~$Xf~",$n["type"]))$I.="/$X";}}if($y&&!preg_match('~set|blob|bytea|raw|file|bool~',$n["type"]))$I.="/SQL";}if($n["auto_increment"]&&!$Fi)$I='è‡ªå‹•éå¢';return
explode("/",$I);}function
editInput($Q,$n,$ya,$Y){if($n["type"]=="enum")return(isset($_GET["select"])?"<label><input type='radio'$ya value='-1' checked><i>".'åŸå§‹'."</i></label> ":"").($n["null"]?"<label><input type='radio'$ya value=''".($Y!==null||isset($_GET["select"])?"":" checked")."><i>NULL</i></label> ":"").enum_input("radio",$ya,$n,$Y,$Y===0?0:null);return"";}function
...
<p id="dbs">
';hidden_fields_get();$Jb=script("mixin(qsl('select'), {onmousedown: dbMouseDown, onchange: dbChange});");echo"<span title='".'è³‡æ–™åº«'."'>".'è³‡æ–™åº«'.":</span> ".($i?html_select("db",array(""=>"")+$i,DB).$Jb:"<input name='db' value='".h(DB)."' autocapitalize='off' size='19'>\n"),"<input type='submit' value='".'ä½¿ç”¨'."'".($i?" class='hidden'":"").">\n";if(support("scheme")){if($Pe!="db"&&DB!=""&&$f->select_db(DB)){echo"<br><span>".'è³‡æ–™è¡¨çµæ§‹'.":</span> ".html_select("ns",array(""=>"")+$b->schemas(),$_GET["ns"]).$Jb;if($_GET["ns"]!="")set_schema($_GET["ns"]);}}foreach(array("import","sql","schema","dump","privileges")as$X){if(isset($_GET[$X])){echo"<input type='hidden' name='$X' value=''>";break;}}echo"</p></form>\n";}function
...
edit_type($y,$n,$gb,$cd=array(),$Kc=array()){global$l;$U=$n["type"];echo"<td><select name='".h($y)."[type]' class='type' aria-labelledby='label-type'>";if($U&&!array_key_exists($U,$l->types())&&!isset($cd[$U])&&!in_array($U,$Kc))$Kc[]=$U;$Ch=$l->structuredTypes();if($cd)$Ch['å¤–ä¾†éµ']=$cd;echo
...
as$y=>$X)echo"<tr valign='top'><td>".h($X[0])."<td>".h($X[1])."<th>".h($y)."<td><a href='".h(ME.'trigger='.urlencode($a).'&name='.urlencode($y))."'>".'ä¿®æ”¹'."</a>\n";echo"</table>\n";}echo'<p class="links"><a href="'.h(ME).'trigger='.urlencode($a).'">'.'å»ºç«‹è§¸ç™¼å™¨'."</a>\n";}}elseif(isset($_GET["schema"])){page_header('è³‡æ–™åº«çµæ§‹',"",array(),h(DB.($_GET["ns"]?".$_GET[ns]":"")));$Nh=array();$Oh=array();$ea=($_GET["schema"]?:$_COOKIE["adminer_schema-".str_replace(".","_",DB)]);preg_match_all('~([^:]+):([-0-9.]+)x([-0-9.]+)(_|$)~',$ea,$Ae,PREG_SET_ORDER);foreach($Ae
...
<table class="layout">
';$Kb=array('','USE','DROP+CREATE','CREATE');$Ph=array('','DROP+CREATE','CREATE');$Hb=array('','TRUNCATE+INSERT','INSERT');if(JUSH=="sql")$Hb[]='INSERT+UPDATE';$J=get_settings("adminer_export");if(!$J)$J=array("output"=>"text","format"=>"sql","db_style"=>(DB!=""?"":"CREATE"),"table_style"=>"DROP+CREATE","data_style"=>"INSERT");if(!isset($J["events"])){$J["routines"]=$J["events"]=($_GET["dump"]=="");$J["triggers"]=$J["table_style"];}echo"<tr><th>".'è¼¸å‡º'."<td>".html_radios("output",$b->dumpOutput(),$J["output"])."\n","<tr><th>".'æ ¼å¼'."<td>".html_radios("format",$b->dumpFormat(),$J["format"])."\n",(JUSH=="sqlite"?"":"<tr><th>".'è³‡æ–™åº«'."<td>".html_select('db_style',$Kb,$J["db_style"]).(support("type")?checkbox("types",1,$J["types"],'ä½¿ç”¨è€…é¡å‹'):"").(support("routine")?checkbox("routines",1,$J["routines"],'ç¨‹åº'):"").(support("event")?checkbox("events",1,$J["events"],'äº‹ä»¶'):"")),"<tr><th>".'è³‡æ–™è¡¨'."<td>".html_select('table_style',$Ph,$J["table_style"]).checkbox("auto_increment",1,$J["auto_increment"],'è‡ªå‹•éå¢').(support("trigger")?checkbox("triggers",1,$J["triggers"],'è§¸ç™¼å™¨'):""),"<tr><th>".'è³‡æ–™'."<td>".html_select('data_style',$Hb,$J["data_style"]),'</table>
<p><input type="submit" value="åŒ¯å‡º">
<input type="hidden" name="token" value="',$T,'">

<table>
',script("qsl('table').onclick = dumpClick;");$ig=array();if(DB!=""){$Wa=($a!=""?"":" checked");echo"<thead><tr>","<th style='text-align: left;'><label class='block'><input type='checkbox' id='check-tables'$Wa>".'è³‡æ–™è¡¨'."</label>".script("qs('#check-tables').onclick = partial(formCheck, /^tables\\[/);",""),"<th style='text-align: right;'><label class='block'>".'è³‡æ–™'."<input type='checkbox' id='check-data'$Wa></label>".script("qs('#check-data').onclick = partial(formCheck, /^data\\[/);",""),"</thead>\n";$Vi="";$Qh=tables_list();foreach($Qh
...
';$Tc=true;foreach($ig
as$y=>$X){if($y!=""&&$X>1){echo($Tc?"<p>":" ")."<a href='".h(ME)."dump=".urlencode("$y%")."'>".h($y)."</a>";$Tc=false;}}}elseif(isset($_GET["privileges"])){page_header('æ¬Šé™');echo'<p class="links"><a href="'.h(ME).'user=">'.'å»ºç«‹ä½¿ç”¨è€…'."</a>";$H=$f->query("SELECT User, Host FROM mysql.".(DB==""?"user":"db WHERE ".q(DB)." LIKE Db")." ORDER BY Host, User");$ld=$H;if(!$H)$H=$f->query("SELECT SUBSTRING_INDEX(CURRENT_USER, '@', 1) AS User, SUBSTRING_INDEX(CURRENT_USER, '@', -1) AS Host");echo"<form action=''><p>\n";hidden_fields_get();echo"<input type='hidden' name='db' value='".h(DB)."'>\n",($ld?"":"<input type='hidden' name='grant' value=''>\n"),"<table class='odds'>\n","<thead><tr><th>".'å¸³è™Ÿ'."<th>".'ä¼ºæœå™¨'."<th></thead>\n";while($J=$H->fetch_assoc())echo'<tr><td>'.h($J["User"])."<td>".h($J["Host"]).'<td><a href="'.h(ME.'user='.urlencode($J["User"]).'&host='.urlencode($J["Host"])).'">'.'ç·¨è¼¯'."</a>\n";if(!$ld||DB!="")echo"<tr><td><input name='user' autocapitalize='off'><td><input name='host' value='localhost' autocapitalize='off'><td><input type='submit' value='".'ç·¨è¼¯'."'>\n";echo"</table>\n","</form>\n";}elseif(isset($_GET["sql"])){if(!$m&&$_POST["export"]){save_settings(array("output"=>$_POST["output"],"format"=>$_POST["format"]),"adminer_import");dump_headers("sql");$b->dumpTable("","");$b->dumpData("","table",$_POST["query"]);$b->dumpFooter();exit;}restart_session();$_d=&get_session("queries");$zd=&$_d[DB];if(!$m&&$_POST["clear"]){$zd=array();redirect(remove_from_uri("history"));}page_header((isset($_GET["import"])?'åŒ¯å…¥':'SQL å‘½ä»¤'),$m);if(!$m&&$_POST){$r=false;if(!isset($_GET["import"]))$G=$_POST["query"];elseif($_POST["webfile"]){$vh=$b->importServerPath();$r=@fopen((file_exists($vh)?$vh:"compress.zlib://$vh.gz"),"rb");$G=($r?fread($r,1e6):false);}else$G=get_file("sql_file",true,";");if(is_string($G)){if(function_exists('memory_get_usage')&&($Ie=ini_bytes("memory_limit"))!="-1")@ini_set("memory_limit",max($Ie,2*strlen($G)+memory_get_usage()+8e6));if($G!=""&&strlen($G)<1e6){$rg=$G.(preg_match("~;[ \t\r\n]*\$~",$G)?"":";");if(!$zd||reset(end($zd))!=$rg){restart_session();$zd[]=array($rg,time());set_session("queries",$_d);stop_session();}}$sh="(?:\\s|/\\*[\s\S]*?\\*/|(?:#|-- )[^\n]*\n?|--\r?\n)";$Qb=";";$C=0;$pc=true;$g=connect($b->credentials());if(is_object($g)&&DB!=""){$g->select_db(DB);if($_GET["ns"]!="")set_schema($_GET["ns"],$g);}$kb=0;$xc=array();$Pf='[\'"'.(JUSH=="sql"?'`#':(JUSH=="sqlite"?'`[':(JUSH=="mssql"?'[':''))).']|/\*|-- |$'.(JUSH=="pgsql"?'|\$[^$]*\$':'');$ki=microtime(true);$oa=get_settings("adminer_import");$gc=$b->dumpFormat();unset($gc["sql"]);while($G!=""){if(!$C&&preg_match("~^$sh*+DELIMITER\\s+(\\S+)~i",$G,$A)){$Qb=$A[1];$G=substr($G,strlen($A[0]));}else{preg_match('('.preg_quote($Qb)."\\s*|$Pf)",$G,$A,PREG_OFFSET_CAPTURE,$C);list($ed,$cg)=$A[0];if(!$ed&&$r&&!feof($r))$G.=fread($r,1e5);else{if(!$ed&&rtrim($G)=="")break;$C=$cg+strlen($ed);if($ed&&rtrim($ed)!=$Qb){$Pa=$l->hasCStyleEscapes()||(JUSH=="pgsql"&&($cg>0&&strtolower($G[$cg-1])=="e"));$Xf=($ed=='/*'?'\*/':($ed=='['?']':(preg_match('~^-- |^#~',$ed)?"\n":preg_quote($ed).($Pa?"|\\\\.":""))));while(preg_match("($Xf|\$)s",$G,$A,PREG_OFFSET_CAPTURE,$C)){$Qg=$A[0][0];if(!$Qg&&$r&&!feof($r))$G.=fread($r,1e5);else{$C=$A[0][1]+strlen($Qg);if(!$Qg||$Qg[0]!="\\")break;}}}else{$pc=false;$rg=substr($G,0,$cg);$kb++;$kg="<pre id='sql-$kb'><code class='jush-".JUSH."'>".$b->sqlCommandQuery($rg)."</code></pre>\n";if(JUSH=="sqlite"&&preg_match("~^$sh*+ATTACH\\b~i",$rg,$A)){echo$kg,"<p class='error'>".'ä¸æ”¯æ´ATTACHæŸ¥è©¢ã€‚'."\n";$xc[]=" <a href='#sql-$kb'>$kb</a>";if($_POST["error_stops"])break;}else{if(!$_POST["only_errors"]){echo$kg;ob_flush();flush();}$_h=microtime(true);if($f->multi_query($rg)&&is_object($g)&&preg_match("~^$sh*+USE\\b~i",$rg))$g->query($rg);do{$H=$f->store_result();if($f->error){echo($_POST["only_errors"]?$kg:""),"<p class='error'>".'æŸ¥è©¢ç™¼ç”ŸéŒ¯èª¤'.($f->errno?" ($f->errno)":"").": ".error()."\n";$xc[]=" <a href='#sql-$kb'>$kb</a>";if($_POST["error_stops"])break
2;}else{$ai=" <span class='time'>(".format_time($_h).")</span>".(strlen($rg)<1000?" <a href='".h(ME)."sql=".urlencode(trim($rg))."'>".'ç·¨è¼¯'."</a>":"");$qa=$f->affected_rows;$Yi=($_POST["only_errors"]?"":$l->warnings());$Zi="warnings-$kb";if($Yi)$ai.=", <a href='#$Zi'>".'è­¦å‘Š'."</a>".script("qsl('a').onclick = partial(toggle, '$Zi');","");$Ec=null;$Fc="explain-$kb";if(is_object($H)){$z=$_POST["limit"];$Af=select($H,$g,array(),$z);if(!$_POST["only_errors"]){echo"<form action='' method='post'>\n";$cf=$H->num_rows;echo"<p>".($cf?($z&&$cf>$z?sprintf('%d / ',$z):"").sprintf('%d è¡Œ',$cf):""),$ai;if($g&&preg_match("~^($sh|\\()*+SELECT\\b~i",$rg)&&($Ec=explain($g,$rg)))echo", <a href='#$Fc'>Explain</a>".script("qsl('a').onclick = partial(toggle, '$Fc');","");$u="export-$kb";echo", <a href='#$u'>".'åŒ¯å‡º'."</a>".script("qsl('a').onclick = partial(toggle, '$u');","")."<span id='$u' class='hidden'>: ".html_select("output",$b->dumpOutput(),$oa["output"])." ".html_select("format",$gc,$oa["format"])."<input type='hidden' name='query' value='".h($rg)."'>"." <input type='submit' name='export' value='".'åŒ¯å‡º'."'><input type='hidden' name='token' value='$T'></span>\n"."</form>\n";}}else{if(preg_match("~^$sh*+(CREATE|DROP|ALTER)$sh++(DATABASE|SCHEMA)\\b~i",$rg)){restart_session();set_session("dbs",null);stop_session();}if(!$_POST["only_errors"])echo"<p class='message' title='".h($f->info)."'>".sprintf('åŸ·è¡ŒæŸ¥è©¢ OKï¼Œ%d è¡Œå—å½±éŸ¿ã€‚',$qa)."$ai\n";}echo($Yi?"<div id='$Zi' class='hidden'>\n$Yi</div>\n":"");if($Ec){echo"<div id='$Fc' class='hidden explain'>\n";select($Ec,$g,$Af);echo"</div>\n";}}$_h=microtime(true);}while($f->next_result());}$G=substr($G,$C);$C=0;}}}}if($pc)echo"<p class='message'>".'æ²’æœ‰å‘½ä»¤å¯åŸ·è¡Œã€‚'."\n";elseif($_POST["only_errors"])echo"<p class='message'>".sprintf('å·²é †åˆ©åŸ·è¡Œ %d å€‹æŸ¥è©¢ã€‚',$kb-count($xc))," <span class='time'>(".format_time($ki).")</span>\n";elseif($xc&&$kb>1)echo"<p class='error'>".'æŸ¥è©¢ç™¼ç”ŸéŒ¯èª¤'.": ".implode("",$xc)."\n";}else
echo"<p class='error'>".upload_error($G)."\n";}echo'
<form action="" method="post" enctype="multipart/form-data" id="form">
';$Cc="<input type='submit' value='".'åŸ·è¡Œ'."' title='Ctrl+Enter'>";if(!isset($_GET["import"])){$rg=$_GET["sql"];if($_POST)$rg=$_POST["query"];elseif($_GET["history"]=="all")$rg=$zd;elseif($_GET["history"]!="")$rg=$zd[$_GET["history"]][0];echo"<p>";textarea("query",$rg,20);echo
...
as$B=>$n){if(!isset($n["privileges"][$Fi?"update":"insert"])||$b->fieldName($n)==""||$n["generated"])unset($o[$B]);}if($_POST&&!$m&&!isset($_GET["select"])){$ue=$_POST["referer"];if($_POST["insert"])$ue=($Fi?null:$_SERVER["REQUEST_URI"]);elseif(!preg_match('~^.+&select=.+$~',$ue))$ue=ME."select=".urlencode($a);$x=indexes($a);$Ai=unique_array($_GET["where"],$x);$ug="\nWHERE $Z";if(isset($_POST["delete"]))queries_redirect($ue,'è©²é …ç›®å·²è¢«åˆªé™¤ã€‚',$l->delete($a,$ug,!$Ai));else{$N=array();foreach($o
as$B=>$n){$X=process_input($n);if($X!==false&&$X!==null)$N[idf_escape($B)]=$X;}if($Fi){if(!$N)redirect($ue);queries_redirect($ue,'å·²æ›´æ–°é …ç›®ã€‚',$l->update($a,$N,$ug,!$Ai));if(is_ajax()){page_headers();page_messages($m);exit;}}else{$H=$l->insert($a,$N);$le=($H?last_id():0);queries_redirect($ue,sprintf('å·²æ–°å¢é …ç›® %sã€‚',($le?" $le":"")),$H);}}}$J=null;if($_POST["save"])$J=(array)$_POST["fields"];elseif($Z){$L=array();foreach($o
...
<th><input type="submit" class="wayoff">','æ¬„ä½'.($re?"<span class='idxopts".($lh?"":" hidden")."'> (".'é•·åº¦'.")</span>":"");if($re||support("descidx"))echo
checkbox("options",1,$lh,'é¸é …',"indexOptionsShow(this.checked)","jsonly")."\n";echo'<th id="label-name">åç¨±
<th><noscript>',"<input type='image' class='icon' name='add[0]' src='".h(preg_replace("~\\?.*~","",ME)."?file=plus.gif&version=5.0.6")."' alt='+' title='".'æ–°å¢ä¸‹ä¸€ç­†'."'>",'</noscript>
</thead>
';if($F){echo"<tr><td>PRIMARY<td>";foreach($F["columns"]as$y=>$d)echo
select_input(" disabled",$o,$d),"<label><input disabled type='checkbox'>".'é™å†ª (éæ¸›)'."</label> ";echo"<td><td>\n";}$ae=1;foreach($J["indexes"]as$w){if(!$_POST["drop_col"]||$ae!=key($_POST["drop_col"])){echo"<tr><td>".html_select("indexes[$ae][type]",array(-1=>"")+$Kd,$w["type"],($ae==count($J["indexes"])?"indexesAddRow.call(this);":""),"label-type"),"<td>";ksort($w["columns"]);$t=1;foreach($w["columns"]as$y=>$d){echo"<span>".select_input(" name='indexes[$ae][columns][$t]' title='".'æ¬„ä½'."'",($o?array_combine($o,$o):$o),$d,"partial(".($t==count($w["columns"])?"indexesAddColumn":"indexesChangeColumn").", '".js_escape(JUSH=="sql"?"":$_GET["indexes"]."_")."')"),"<span class='idxopts".($lh?"":" hidden")."'>",($re?"<input type='number' name='indexes[$ae][lengths][$t]' class='size' value='".h($w["lengths"][$y])."' title='".'é•·åº¦'."'>":""),(support("descidx")?checkbox("indexes[$ae][descs][$t]",1,$w["descs"][$y],'é™å†ª (éæ¸›)'):""),"</span> </span>";$t++;}echo"<td><input name='indexes[$ae][name]' value='".h($w["name"])."' autocapitalize='off' aria-labelledby='label-name'>\n","<td><input type='image' class='icon' name='drop_col[$ae]' src='".h(preg_replace("~\\?.*~","",ME)."?file=cross.gif&version=5.0.6")."' alt='x' title='".'ç§»é™¤'."'>".script("qsl('input').onclick = partial(editingRemoveRow, 'indexes\$1[type]');");}$ae++;}echo'</table>
</div>
<p>
<input type="submit" value="å„²å­˜">
<input type="hidden" name="token" value="',$T,'">
...
as$j){if(count($i)==1||$j!=""){if(!create_database($j,$J["collation"]))$Eh=false;$ke=$j;}}restart_session();set_session("dbs",null);queries_redirect(ME."db=".urlencode($ke),'å·²å»ºç«‹è³‡æ–™åº«ã€‚',$Eh);}}else{if(!$J["collation"])redirect(substr(ME,0,-1));query_redirect("ALTER DATABASE ".idf_escape($B).(preg_match('~^[a-z0-9_]+$~i',$J["collation"])?" COLLATE $J[collation]":""),substr(ME,0,-1),'å·²ä¿®æ”¹è³‡æ–™åº«ã€‚');}}page_header(DB!=""?'ä¿®æ”¹è³‡æ–™åº«':'å»ºç«‹è³‡æ–™åº«',$m,array(),h(DB));$gb=collations();$B=DB;if($_POST)$B=$J["name"];elseif(DB!="")$J["collation"]=db_collation(DB,$gb);elseif(JUSH=="sql"){foreach(get_vals("SHOW GRANTS")as$ld){if(preg_match('~ ON (`(([^\\\\`]|``|\\\\.)*)%`\.\*)?~',$ld,$A)&&$A[1]){$B=stripcslashes(idf_unescape("`$A[2]`"));break;}}}echo'
<form action="" method="post">
<p>
',($_POST["add_x"]||strpos($B,"\n")?'<textarea autofocus name="name" rows="10" cols="40">'.h($B).'</textarea><br>':'<input name="name" autofocus value="'.h($B).'" data-maxlength="64" autocapitalize="off">')."\n".($gb?html_select("collation",array(""=>"(".'æ ¡å°'.")")+$gb,$J["collation"]).doc_link(array('sql'=>"charset-charsets.html",'mariadb'=>"supported-character-sets-and-collations/",'mssql'=>"relational-databases/system-functions/sys-fn-helpcollations-transact-sql",)):""),'<input type="submit" value="å„²å­˜">
';if(DB!="")echo"<input type='submit' name='drop' value='".'åˆªé™¤'."'>".confirm(sprintf('åˆªé™¤ %s?',DB))."\n";elseif(!$_POST["add_x"]&&$_GET["db"]=="")echo"<input type='image' class='icon' name='add' src='".h(preg_replace("~\\?.*~","",ME)."?file=plus.gif&version=5.0.6")."' alt='+' title='".'æ–°å¢ä¸‹ä¸€ç­†'."'>\n";echo'<input type="hidden" name="token" value="',$T,'">
</form>
...
';}elseif(isset($_GET["foreign"])){$a=$_GET["foreign"];$B=$_GET["name"];$J=$_POST;if($_POST&&!$m&&!$_POST["add"]&&!$_POST["change"]&&!$_POST["change-js"]){if(!$_POST["drop"]){$J["source"]=array_filter($J["source"],'strlen');ksort($J["source"]);$Th=array();foreach($J["source"]as$y=>$X)$Th[$y]=$J["target"][$y];$J["target"]=$Th;}if(JUSH=="sqlite")$H=recreate_table($a,$a,array(),array(),array(" $B"=>($J["drop"]?"":" ".format_foreign_key($J))));else{$c="ALTER TABLE ".table($a);$H=($B==""||queries("$c DROP ".(JUSH=="sql"?"FOREIGN KEY ":"CONSTRAINT ").idf_escape($B)));if(!$J["drop"])$H=queries("$c ADD".format_foreign_key($J));}queries_redirect(ME."table=".urlencode($a),($J["drop"]?'å·²åˆªé™¤å¤–ä¾†éµã€‚':($B!=""?'å·²ä¿®æ”¹å¤–ä¾†éµã€‚':'å·²å»ºç«‹å¤–ä¾†éµã€‚')),$H);if(!$J["drop"])$m="$m<br>".'ä¾†æºåˆ—å’Œç›®æ¨™åˆ—å¿…é ˆå…·æœ‰ç›¸åŒçš„è³‡æ–™é¡å‹ï¼Œåœ¨ç›®æ¨™åˆ—ä¸Šå¿…é ˆæœ‰ä¸€å€‹ç´¢å¼•ä¸¦ä¸”å¼•ç”¨çš„è³‡æ–™å¿…é ˆå­˜åœ¨ã€‚';}page_header('å¤–ä¾†éµ',$m,array("table"=>$a),h($a));if($_POST){ksort($J["source"]);if($_POST["add"])$J["source"][]="";elseif($_POST["change"]||$_POST["change-js"])$J["target"]=array();}elseif($B!=""){$cd=foreign_keys($a);$J=$cd[$B];$J["source"][]="";}else{$J["table"]=$a;$J["source"]=array("");}echo'
<form action="" method="post">
';$rh=array_keys(fields($a));if($J["db"]!="")$f->select_db($J["db"]);if($J["ns"]!=""){$Ef=get_schema();set_schema($J["ns"]);}$zg=array_keys(array_filter(table_status('',true),'Adminer\fk_support'));$Th=array_keys(fields(in_array($J["table"],$zg)?$J["table"]:reset($zg)));$pf="this.form['change-js'].value = '1'; this.form.submit();";echo"<p>".'ç›®æ¨™è³‡æ–™è¡¨'.": ".html_select("table",$zg,$J["table"],$pf)."\n";if(support("scheme")){$Tg=array_filter($b->schemas(),function($Sg){return!preg_match('~^information_schema$~i',$Sg);});echo'è³‡æ–™è¡¨çµæ§‹'.": ".html_select("ns",$Tg,$J["ns"]!=""?$J["ns"]:$_GET["ns"],$pf);if($J["ns"]!="")set_schema($Ef);}elseif(JUSH!="sqlite"){$Lb=array();foreach($b->databases()as$j){if(!information_schema($j))$Lb[]=$j;}echo'è³‡æ–™åº«'.": ".html_select("db",$Lb,$J["db"]!=""?$J["db"]:$_GET["db"],$pf);}echo'<input type="hidden" name="change-js" value="">
...
';}elseif(isset($_GET["view"])){$a=$_GET["view"];$J=$_POST;$Ff="VIEW";if(JUSH=="pgsql"&&$a!=""){$O=table_status($a);$Ff=strtoupper($O["Engine"]);}if($_POST&&!$m){$B=trim($J["name"]);$wa=" AS\n$J[select]";$ue=ME."table=".urlencode($B);$Je='å·²ä¿®æ”¹æª¢è¦–è¡¨ã€‚';$U=($_POST["materialized"]?"MATERIALIZED VIEW":"VIEW");if(!$_POST["drop"]&&$a==$B&&JUSH!="sqlite"&&$U=="VIEW"&&$Ff=="VIEW")query_redirect((JUSH=="mssql"?"ALTER":"CREATE OR REPLACE")." VIEW ".table($B).$wa,$ue,$Je);else{$Vh=$B."_adminer_".uniqid();drop_create("DROP $Ff ".table($a),"CREATE $U ".table($B).$wa,"DROP $U ".table($B),"CREATE $U ".table($Vh).$wa,"DROP $U ".table($Vh),($_POST["drop"]?substr(ME,0,-1):$ue),'å·²åˆªé™¤æª¢è¦–è¡¨ã€‚',$Je,'å·²å»ºç«‹æª¢è¦–è¡¨ã€‚',$a,$B);}}if(!$_POST&&$a!=""){$J=view($a);$J["name"]=$a;$J["materialized"]=($Ff!="VIEW");if(!$m)$m=error();}page_header(($a!=""?'ä¿®æ”¹æª¢è¦–è¡¨':'å»ºç«‹æª¢è¦–è¡¨'),$m,array("table"=>$a),h($a));echo'
<form action="" method="post">
...
';}elseif(isset($_GET["event"])){$aa=$_GET["event"];$Sd=array("YEAR","QUARTER","MONTH","DAY","HOUR","MINUTE","WEEK","SECOND","YEAR_MONTH","DAY_HOUR","DAY_MINUTE","DAY_SECOND","HOUR_MINUTE","HOUR_SECOND","MINUTE_SECOND");$Ah=array("ENABLED"=>"ENABLE","DISABLED"=>"DISABLE","SLAVESIDE_DISABLED"=>"DISABLE ON SLAVE");$J=$_POST;if($_POST&&!$m){if($_POST["drop"])query_redirect("DROP EVENT ".idf_escape($aa),substr(ME,0,-1),'å·²åˆªé™¤äº‹ä»¶ã€‚');elseif(in_array($J["INTERVAL_FIELD"],$Sd)&&isset($Ah[$J["STATUS"]])){$Rg="\nON SCHEDULE ".($J["INTERVAL_VALUE"]?"EVERY ".q($J["INTERVAL_VALUE"])." $J[INTERVAL_FIELD]".($J["STARTS"]?" STARTS ".q($J["STARTS"]):"").($J["ENDS"]?" ENDS ".q($J["ENDS"]):""):"AT ".q($J["STARTS"]))." ON COMPLETION".($J["ON_COMPLETION"]?"":" NOT")." PRESERVE";queries_redirect(substr(ME,0,-1),($aa!=""?'å·²ä¿®æ”¹äº‹ä»¶ã€‚':'å·²å»ºç«‹äº‹ä»¶ã€‚'),queries(($aa!=""?"ALTER EVENT ".idf_escape($aa).$Rg.($aa!=$J["EVENT_NAME"]?"\nRENAME TO ".idf_escape($J["EVENT_NAME"]):""):"CREATE EVENT ".idf_escape($J["EVENT_NAME"]).$Rg)."\n".$Ah[$J["STATUS"]]." COMMENT ".q($J["EVENT_COMMENT"]).rtrim(" DO\n$J[EVENT_DEFINITION]",";").";"));}}page_header(($aa!=""?'ä¿®æ”¹äº‹ä»¶'.": ".h($aa):'å»ºç«‹äº‹ä»¶'),$m);if(!$J&&$aa!=""){$K=get_rows("SELECT * FROM information_schema.EVENTS WHERE EVENT_SCHEMA = ".q(DB)." AND EVENT_NAME = ".q($aa));$J=reset($K);}echo'
<form action="" method="post">
<table class="layout">
...
<tr><th>æ¯<td><input type="number" name="INTERVAL_VALUE" value="',h($J["INTERVAL_VALUE"]),'" class="size"> ',html_select("INTERVAL_FIELD",$Sd,$J["INTERVAL_FIELD"]),'<tr><th>ç‹€æ…‹<td>',html_select("STATUS",$Ah,$J["STATUS"]),'<tr><th>è¨»è§£<td><input name="EVENT_COMMENT" value="',h($J["EVENT_COMMENT"]),'" data-maxlength="64">
...
';}elseif(isset($_GET["procedure"])){$da=($_GET["name"]?:$_GET["procedure"]);$Mg=(isset($_GET["function"])?"FUNCTION":"PROCEDURE");$J=$_POST;$J["fields"]=(array)$J["fields"];if($_POST&&!process_fields($J["fields"])&&!$m){$Bf=routine($_GET["procedure"],$Mg);$Vh="$J[name]_adminer_".uniqid();drop_create("DROP $Mg ".routine_id($da,$Bf),create_routine($Mg,$J),"DROP $Mg ".routine_id($J["name"],$J),create_routine($Mg,array("name"=>$Vh)+$J),"DROP $Mg ".routine_id($Vh,$J),substr(ME,0,-1),'å·²åˆªé™¤ç¨‹åºã€‚','å·²ä¿®æ”¹å­ç¨‹åºã€‚','å·²å»ºç«‹å­ç¨‹åºã€‚',$da,$J["name"]);}page_header(($da!=""?(isset($_GET["function"])?'ä¿®æ”¹å‡½å¼':'ä¿®æ”¹é å­˜ç¨‹åº').": ".h($da):(isset($_GET["function"])?'å»ºç«‹å‡½å¼':'å»ºç«‹é å­˜ç¨‹åº')),$m);if(!$_POST&&$da!=""){$J=routine($_GET["procedure"],$Mg);$J["name"]=$da;}$gb=get_vals("SHOW CHARACTER SET");sort($gb);$Ng=routine_languages();echo($gb?"<datalist id='collations'>".optionlist($gb)."</datalist>":""),'
<form action="" method="post" id="form">
<p>åç¨±: <input name="name" value="',h($J["name"]),'" data-maxlength="64" autocapitalize="off">
',($Ng?'èªè¨€'.": ".html_select("language",$Ng,$J["language"])."\n":""),'<input type="submit" value="å„²å­˜">
<div class="scrollable">
<table class="nowrap">
';edit_fields($J["fields"],$gb,$Mg);if(isset($_GET["function"])){echo"<tr><td>".'å›å‚³é¡å‹';edit_type("returns",$J["returns"],$gb,array(),(JUSH=="pgsql"?array("void","trigger"):array()));}echo'</table>
',script("editFields();"),'</div>
...
';}elseif(isset($_GET["trigger"])){$a=$_GET["trigger"];$B=$_GET["name"];$ti=trigger_options();$J=(array)trigger($B,$a)+array("Trigger"=>$a."_bi");if($_POST){if(!$m&&in_array($_POST["Timing"],$ti["Timing"])&&in_array($_POST["Event"],$ti["Event"])&&in_array($_POST["Type"],$ti["Type"])){$mf=" ON ".table($a);$ac="DROP TRIGGER ".idf_escape($B).(JUSH=="pgsql"?$mf:"");$ue=ME."table=".urlencode($a);if($_POST["drop"])query_redirect($ac,$ue,'å·²åˆªé™¤è§¸ç™¼å™¨ã€‚');else{if($B!="")queries($ac);queries_redirect($ue,($B!=""?'å·²ä¿®æ”¹è§¸ç™¼å™¨ã€‚':'å·²å»ºç«‹è§¸ç™¼å™¨ã€‚'),queries(create_trigger($mf,$_POST)));if($B!="")queries(create_trigger($mf,$J+array("Type"=>reset($ti["Type"]))));}}$J=$_POST;}page_header(($B!=""?'ä¿®æ”¹è§¸ç™¼å™¨'.": ".h($B):'å»ºç«‹è§¸ç™¼å™¨'),$m,array("table"=>$a));echo'
<form action="" method="post" id="form">
<table class="layout">
<tr><th>æ™‚é–“<td>',html_select("Timing",$ti["Timing"],$J["Timing"],"triggerChange(/^".preg_quote($a,"/")."_[ba][iud]$/, '".js_escape($a)."', this.form);"),'<tr><th>äº‹ä»¶<td>',html_select("Event",$ti["Event"],$J["Event"],"this.form['Timing'].onchange();"),(in_array("UPDATE OF",$ti["Event"])?" <input name='Of' value='".h($J["Of"])."' class='hidden'>":""),'<tr><th>é¡å‹<td>',html_select("Type",$ti["Type"],$J["Type"]),'</table>
...
as$ef=>$Jg){if(preg_match('~^(.+)(\(.*\))?$~U',$ef,$A))grant("REVOKE",array_keys($Jg),$A[2]," ON $A[1] FROM $Ye");}}}queries_redirect(ME."privileges=",(isset($_GET["host"])?'å·²ä¿®æ”¹ä½¿ç”¨è€…ã€‚':'å·²å»ºç«‹ä½¿ç”¨è€…ã€‚'),!$m);if($_b)$f->query("DROP USER $Ye");}}page_header((isset($_GET["host"])?'å¸³è™Ÿ'.": ".h("$ha@$_GET[host]"):'å»ºç«‹ä½¿ç”¨è€…'),$m,array("privileges"=>array('','æ¬Šé™')));$J=$_POST;if($J)$md=$We;else{$J=$_GET+array("host"=>get_val("SELECT SUBSTRING_INDEX(CURRENT_USER, '@', -1)"));$J["pass"]=$kf;if($kf!="")$J["hashed"]=true;$md[(DB==""||$md?"":idf_escape(addcslashes(DB,"%_\\"))).".*"]=array();}echo'<form action="" method="post">
<table class="layout">
...
as$ef=>$ld){echo'<th>'.($ef!="*.*"?"<input name='objects[$t]' value='".h($ef)."' size='10' autocapitalize='off'>":"<input type='hidden' name='objects[$t]' value='*.*' size='10'>*.*");$t++;}echo"</thead>\n";foreach(array(""=>"","Server Admin"=>'ä¼ºæœå™¨',"Databases"=>'è³‡æ–™åº«',"Tables"=>'è³‡æ–™è¡¨',"Columns"=>'æ¬„ä½',"Procedures"=>'ç¨‹åº',)as$vb=>$Rb){foreach((array)$ng[$vb]as$mg=>$lb){echo"<tr><td".($Rb?">$Rb<td":" colspan='2'").' lang="en" title="'.h($lb).'">'.h($mg);$t=0;foreach($md
as$ef=>$ld){$B="'grants[$t][".h(strtoupper($mg))."]'";$Y=$ld[strtoupper($mg)];if($vb=="Server Admin"&&$ef!=(isset($md["*.*"])?"*.*":".*"))echo"<td>";elseif(isset($_GET["grant"]))echo"<td><select name=$B><option><option value='1'".($Y?" selected":"").">".'æˆæ¬Š'."<option value='0'".($Y=="0"?" selected":"").">".'å»¢é™¤'."</select>";else
...
';}elseif(isset($_GET["processlist"])){if(support("kill")){if($_POST&&!$m){$ge=0;foreach((array)$_POST["kill"]as$X){if(kill_process($X))$ge++;}queries_redirect(ME."processlist=",sprintf('%d å€‹ Process(es) è¢«çµ‚æ­¢ã€‚',$ge),$ge||!$_POST["kill"]);}}page_header('è™•ç†ç¨‹åºåˆ—è¡¨',$m);echo'
<form action="" method="post">
<div class="scrollable">
...
as$ce=>$W)$_.=where_link($t++,$ce,$W);}$X=select_value($X,$_,$n,$Zh);$u=h("val[$Bi][".bracket_escape($y)."]");$Y=$_POST["val"][$Bi][bracket_escape($y)];$jc=!is_array($J[$y])&&is_utf8($X)&&$K[$Te][$y]==$J[$y]&&!$id[$y]&&!$n["generated"];$Xh=preg_match('~text|json|lob~',$n["type"]);echo"<td id='$u'".(preg_match(number_type(),$n["type"])&&is_numeric(strip_tags($X))?" class='number'":"");if(($_GET["modify"]&&$jc)||$Y!==null){$sd=h($Y!==null?$Y:$J[$y]);echo">".($Xh?"<textarea name='$u' cols='30' rows='".(substr_count($J[$y],"\n")+1)."'>$sd</textarea>":"<input name='$u' value='$sd' size='$re[$y]'>");}else{$we=strpos($X,"<i>â€¦</i>");echo" data-text='".($we?2:($Xh?1:0))."'".($jc?"":" data-warning='".h('ä½¿ç”¨ç·¨è¼¯é€£çµä¾†ä¿®æ”¹ã€‚')."'").">$X";}}}if($Ea)echo"<td>";$b->backwardKeysPrint($Ea,$K[$Te]);echo"</tr>\n";}if(is_ajax())exit;echo"</table>\n","</div>\n";}if(!is_ajax()){if($K||$D){$Bc=true;if($_GET["page"]!="last"){if($z==""||(count($K)<$z&&($K||!$D)))$fd=($D?$D*$z:0)+count($K);elseif(JUSH!="sql"||!$Wd){$fd=($Wd?false:found_rows($R,$Z));if($fd<max(1e4,2*($D+1)*$z))$fd=reset(slow_query(count_rows($a,$Z,$Wd,$nd)));else$Bc=false;}}$Mf=($z!=""&&($fd===false||$fd>$z||$D));if($Mf)echo(($fd===false?count($K)+1:$fd-$D*$z)>$z?'<p><a href="'.h(remove_from_uri("page")."&page=".($D+1)).'" class="loadmore">'.'è¼‰å…¥æ›´å¤šè³‡æ–™'.'</a>'.script("qsl('a').onclick = partial(selectLoadMore, ".(+$z).", '".'è¼‰å…¥ä¸­'."â€¦');",""):''),"\n";}echo"<div class='footer'><div>\n";if($K||$D){if($Mf){$De=($fd===false?$D+(count($K)>=$z?2:1):floor(($fd-1)/$z));echo"<fieldset>";if(JUSH!="simpledb"){echo"<legend><a href='".h(remove_from_uri("page"))."'>".'é '."</a></legend>",script("qsl('a').onclick = function () { pageClick(this.href, +prompt('".'é '."', '".($D+1)."')); return false; };"),pagination(0,$D).($D>5?" â€¦":"");for($t=max(1,$D-4);$t<min($De,$D+5);$t++)echo
pagination($t,$D);if($De>0)echo($D+5<$De?" â€¦":""),($Bc&&$fd!==false?pagination($De,$D):" <a href='".h(remove_from_uri("page")."&page=last")."' title='~$De'>".'æœ€å¾Œä¸€é '."</a>");}else
echo"<legend>".'é '."</legend>",pagination(0,$D).($D>1?" â€¦":""),($D?pagination($D,$D):""),($De>$D?pagination($D+1,$D).($De>$D+1?" â€¦":""):"");echo"</fieldset>\n";}echo"<fieldset>","<legend>".'æ‰€æœ‰çµæœ'."</legend>";$Xb=($Bc?"":"~ ").$fd;$qf="var checked = formChecked(this, /check/); selectCount('selected', this.checked ? '$Xb' : checked); selectCount('selected2', this.checked || !checked ? '$Xb' : checked);";echo
checkbox("all",1,0,($fd!==false?($Bc?"":"~ ").sprintf('%d è¡Œ',$fd):""),$qf)."\n","</fieldset>\n";if($b->selectCommandPrint())echo'<fieldset',($_GET["modify"]?'':' class="jsonly"'),'><legend>ä¿®æ”¹</legend><div>
...
';$dd=$b->dumpFormat();foreach((array)$_GET["columns"]as$d){if($d["fun"]){unset($dd['sql']);break;}}if($dd){print_fieldset("export",'åŒ¯å‡º'." <span id='selected2'></span>");$Jf=$b->dumpOutput();echo($Jf?html_select("output",$Jf,$pa["output"])." ":""),html_select("format",$dd,$pa["format"])," <input type='submit' name='export' value='".'åŒ¯å‡º'."'>\n","</div></fieldset>\n";}$b->selectEmailPrint(array_filter($oc,'strlen'),$e);}echo"</div></div>\n";if($b->selectImportPrint())echo"<div>","<a href='#import'>".'åŒ¯å…¥'."</a>",script("qsl('a').onclick = partial(toggle, 'import');",""),"<span id='import'".($_POST["import"]?"":" class='hidden'").">: ","<input type='file' name='csv_file'> ",html_select("separator",array("csv"=>"CSV,","csv;"=>"CSV;","tsv"=>"TSV"),$pa["format"])," <input type='submit' name='import' value='".'åŒ¯å…¥'."'>","</span>","</div>";echo"<input type='hidden' name='token' value='$T'>\n","</form>\n",(!$nd&&$L?"":script("tableCheck();"));}}}if(is_ajax()){ob_end_clean();exit;}}elseif(isset($_GET["variables"])){$O=isset($_GET["status"]);page_header($O?'ç‹€æ…‹':'è®Šæ•¸');$Ri=($O?show_status():show_variables());if(!$Ri)echo"<p class='message'>".'æ²’æœ‰è³‡æ–™è¡Œã€‚'."\n";else{echo"<table>\n";foreach($Ri

...
as$B=>$m){$X=process_input($m);if($X!==false&&$X!==null)$M[idf_escape($B)]=$X;}if($Hi){if(!$M)adm_redirect($_);queries_redirect($_,'Item has been updated.',$k->update($a,$M,$vg,!$Ci));if(is_ajax()){page_headers();page_messages($l);exit;}}else{$G=$k->insert($a,$M);$qe=($G?last_id():0);queries_redirect($_,sprintf('Item%s has been inserted.',($qe?" $qe":"")),$G);}}}$I=null;if($_POST["save"])$I=(array)$_POST["fields"];elseif($Z){$K=array();foreach($n
...
adm_redirect($z);}}page_header($_GET["ns"]!=""?'Alter schema':'Create schema',$l);if(!$I)$I["name"]=$_GET["ns"];echo'
<form action="" method="post">

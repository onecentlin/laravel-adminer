...
upload_error($m){$Fe=($m==UPLOAD_ERR_INI_SIZE?ini_get("upload_max_filesize"):0);return($m?'Hochladen von Datei fehlgeschlagen.'.($Fe?" ".sprintf('Maximal erlaubte DateigrÃ¶ÃŸe ist %sB.',$Fe):""):'Datei existiert nicht.');}function
...
script("$Zg.onclick = function () { return confirm('".($Je?js_escape($Je):'Sind Sie sicher?')."'); };","");}function
...
query($G,$yi=false){$H=$this->pdo->query($G);$this->error="";if(!$H){list(,$this->errno,$this->error)=$this->pdo->errorInfo();if(!$this->error)$this->error='Unbekannter Fehler.';return
...
connect($Ab){list(,,$E)=$Ab;if($E!="")return'Die Datenbank unterstÃ¼tzt kein Passwort.';return
new
Db;}function
...
h($f->error);}function
check_sqlite_name($B){global$f;$Ic="db|sdb|sqlite";if(!preg_match("~^[^\\0]*\\.($Ic)\$~",$B)){$f->error=sprintf('Bitte einen der Dateitypen %s benutzen.',str_replace("|",", ",$Ic));return
false;}return
true;}function
create_database($j,$fb){global$f;if(file_exists($j)){$f->error='Datei existiert schon.';return
...
as$j){if(!@unlink($j)){$f->error='Datei existiert schon.';return
false;}}return
true;}function
rename_database($B,$fb){global$f;if(!check_sqlite_name($B))return
false;$f->__construct(":memory:");$f->error='Datei existiert schon.';return@rename(DB,$B);}function
...
__construct($f){parent::__construct($f);$this->types=array('Zahlen'=>array("smallint"=>5,"integer"=>10,"bigint"=>19,"boolean"=>1,"numeric"=>0,"real"=>7,"double precision"=>16,"money"=>20),'Datum und Zeit'=>array("date"=>13,"time"=>17,"timestamp"=>20,"timestamptz"=>21,"interval"=>0),'Zeichenketten'=>array("character"=>0,"character varying"=>0,"text"=>0,"tsquery"=>0,"tsvector"=>0,"uuid"=>0,"xml"=>0),'BinÃ¤r'=>array("bit"=>0,"bit varying"=>0,"bytea"=>0),'Netzwerk'=>array("cidr"=>43,"inet"=>43,"macaddr"=>17,"macaddr8"=>23,"txid_snapshot"=>0),'Geometrie'=>array("box"=>0,"circle"=>0,"line"=>0,"lseg"=>0,"path"=>0,"point"=>0,"polygon"=>0),);if(min_version(9.2,0,$f)){$this->types['Zeichenketten']["json"]=4294967295;if(min_version(9.4,0,$f))$this->types['Zeichenketten']["jsonb"]=4294967295;}$this->editFunctions=array(array("char"=>"md5","date|time"=>"now",),array(number_type()=>"+/-","date|time"=>"+ interval/- interval","char|text"=>"||",));if(min_version(12,0,$f))$this->generated=array("STORED");}function
enumLength($n){$sc=$this->types['Benutzerdefinierte Typen'][$n["type"]];return($sc?type_values($sc):"");}function
setUserTypes($xi){$this->types['Benutzerdefinierte Typen']=array_flip($xi);}function
...
__construct($f){parent::__construct($f);$this->types=array('Zahlen'=>array("number"=>38,"binary_float"=>12,"binary_double"=>21),'Datum und Zeit'=>array("date"=>10,"timestamp"=>29,"interval year"=>12,"interval day"=>28),'Zeichenketten'=>array("char"=>2000,"varchar2"=>4000,"nchar"=>2000,"nvarchar2"=>4000,"clob"=>4294967295,"nclob"=>4294967295),'BinÃ¤r'=>array("raw"=>2000,"long raw"=>2147483648,"blob"=>4294967295,"bfile"=>4294967296),);}function
begin(){return
true;}function
insertUpdate($Q,$K,$F){global$f;foreach($K
as$N){$Fi=array();$Z=array();foreach($N
...
__construct($f){parent::__construct($f);$this->types=array('Zahlen'=>array("tinyint"=>3,"smallint"=>5,"int"=>10,"bigint"=>20,"bit"=>1,"decimal"=>0,"real"=>12,"float"=>53,"smallmoney"=>10,"money"=>20),'Datum und Zeit'=>array("date"=>10,"smalldatetime"=>19,"datetime"=>19,"datetime2"=>19,"time"=>8,"datetimeoffset"=>10),'Zeichenketten'=>array("char"=>8000,"varchar"=>8000,"text"=>2147483647,"nchar"=>4000,"nvarchar"=>4000,"ntext"=>1073741823),'BinÃ¤r'=>array("binary"=>8000,"varbinary"=>8000,"image"=>2147483647),);}function
...
loginForm(){global$Zb;echo"<table class='layout'>\n",$this->loginFormField('driver','<tr><th>'.'Datenbank System'.'<td>',html_select("auth[driver]",$Zb,DRIVER,"loginDriver(this);")),$this->loginFormField('server','<tr><th>'.'Server'.'<td>','<input name="auth[server]" value="'.h(SERVER).'" title="hostname[:port]" placeholder="localhost" autocapitalize="off">'),$this->loginFormField('username','<tr><th>'.'Benutzer'.'<td>','<input name="auth[username]" id="username" autofocus value="'.h($_GET["username"]).'" autocomplete="username" autocapitalize="off">'.script("qs('#username').form['auth[driver]'].onchange();")),$this->loginFormField('password','<tr><th>'.'Passwort'.'<td>','<input type="password" name="auth[password]" autocomplete="current-password">'),$this->loginFormField('db','<tr><th>'.'Datenbank'.'<td>','<input name="auth[db]" value="'.h($_GET["db"]).'" autocapitalize="off">'),"</table>\n","<p><input type='submit' value='".'Login'."'>\n",checkbox("auth[permanent]",1,$_COOKIE["adminer_permanent"],'Passwort speichern')."\n";}function
loginFormField($B,$yd,$Y){return$yd.$Y."\n";}function
login($ve,$E){if($E=="")return
sprintf('Adminer unterstÃ¼tzt den Zugriff auf eine Datenbank ohne Passwort nicht, <a href="https://www.adminer.org/en/password/"%s>mehr Informationen</a>.',target_blank());return
true;}function
tableName($Jh){return
h($Jh["Name"]);}function
fieldName($n,$xf=0){return'<span title="'.h($n["full_type"].($n["comment"]!=""?" : $n[comment]":'')).'">'.h($n["field"]).'</span>';}function
selectLinks($Jh,$N=""){global$l;echo'<p class="links">';$te=array("select"=>'Daten auswÃ¤hlen');if(support("table")||support("indexes"))$te["table"]='Struktur anzeigen';$Zd=false;if(support("table")){$Zd=is_view($Jh);if($Zd)$te["view"]='View Ã¤ndern';else$te["create"]='Tabelle Ã¤ndern';}if($N!==null)$te["edit"]='Neuer Datensatz';$B=$Jh["Name"];foreach($te
...
selectQuery($G,$_h,$Lc=false){global$l;$I="</p>\n";if(!$Lc&&($Yi=$l->warnings())){$u="warnings";$I=", <a href='#$u'>".'Warnungen'."</a>".script("qsl('a').onclick = partial(toggle, '$u');","")."$I<div id='$u' class='hidden'>\n$Yi</div>\n";}return"<p><code class='jush-".JUSH."'>".h(str_replace("\n"," ",$G))."</code> <span class='time'>(".format_time($_h).")</span>".(support("sql")?" <a href='".h(ME)."sql=".urlencode($G)."'>".'Bearbeiten'."</a>":"").$I;}function
...
as$n){echo"<tr><th>".h($n["field"]);$U=h($n["full_type"]);echo"<td><span title='".h($n["collation"])."'>".(in_array($U,(array)$Ch['Benutzerdefinierte Typen'])?"<a href='".h(ME.'type='.urlencode($U))."'>$U</a>":$U)."</span>",($n["null"]?" <i>NULL</i>":""),($n["auto_increment"]?" <i>".'Auto-Inkrement'."</i>":"");$k=h($n["default"]);echo(isset($n["default"])?" <span title='".'Vorgabewert festlegen'."'>[<b>".($n["generated"]?"<code class='jush-".JUSH."'>$k</code>":$k)."</b>]</span>":""),(support("comment")?"<td>".h($n["comment"]):""),"\n";}echo"</table>\n","</div>\n";}function
...
selectColumnsPrint($L,$e){global$l;print_fieldset("select",'Daten zeigen von',$L);$t=0;$L[""]=array();foreach($L
as$y=>$X){$X=$_GET["columns"][$y];$d=select_input(" name='columns[$t][col]'",$e,$X["col"],($y!==""?"selectFieldChange":"selectAddRow"));echo"<div>".($l->functions||$l->grouping?html_select("columns[$t][fun]",array(-1=>"")+array_filter(array('Funktionen'=>$l->functions,'Aggregationen'=>$l->grouping)),$X["fun"]).on_help("getTarget(event).value && getTarget(event).value.replace(/ |\$/, '(') + ')'",1).script("qsl('select').onchange = function () { helpClose();".($y!==""?"":" qsl('select, input', this.parentNode).onchange();")." };","")."($d)":$d)."</div>\n";$t++;}echo"</div></fieldset>\n";}function
selectSearchPrint($Z,$e,$x){print_fieldset("search",'Suchen',$Z);foreach($x
as$t=>$w){if($w["type"]=="FULLTEXT")echo"<div>(<i>".implode("</i>, <i>",array_map('Adminer\h',$w["columns"]))."</i>) AGAINST"," <input type='search' name='fulltext[$t]' value='".h($_GET["fulltext"][$t])."'>",script("qsl('input').oninput = selectFieldChange;",""),checkbox("boolean[$t]",1,isset($_GET["boolean"][$t]),"BOOL"),"</div>\n";}$Ra="this.parentNode.firstChild.onchange();";foreach(array_merge((array)$_GET["where"],array(array()))as$t=>$X){if(!$X||("$X[col]$X[val]"!=""&&in_array($X["op"],$this->operators)))echo"<div>".select_input(" name='where[$t][col]'",$e,$X["col"],($X?"selectFieldChange":"selectAddRow"),"(".'beliebig'.")"),html_select("where[$t][op]",$this->operators,$X["op"],$Ra),"<input type='search' name='where[$t][val]' value='".h($X["val"])."'>",script("mixin(qsl('input'), {oninput: function () { $Ra }, onkeydown: selectSearchKeydown, onsearch: selectSearchSearch});",""),"</div>\n";}echo"</div></fieldset>\n";}function
selectOrderPrint($xf,$e,$x){print_fieldset("sort",'Ordnen',$xf);$t=0;foreach((array)$_GET["order"]as$y=>$X){if($X!=""){echo"<div>".select_input(" name='order[$t]'",$e,$X,"selectFieldChange"),checkbox("desc[$t]",1,isset($_GET["desc"][$y]),'absteigend')."</div>\n";$t++;}}echo"<div>".select_input(" name='order[$t]'",$e,"","selectAddRow"),checkbox("desc[$t]",1,false,'absteigend')."</div>\n","</div></fieldset>\n";}function
selectLimitPrint($z){echo"<fieldset><legend>".'Begrenzung'."</legend><div>","<input type='number' name='limit' class='size' value='".h($z)."'>",script("qsl('input').oninput = selectFieldChange;",""),"</div></fieldset>\n";}function
selectLengthPrint($Zh){if($Zh!==null)echo"<fieldset><legend>".'TextlÃ¤nge'."</legend><div>","<input type='number' name='text_length' class='size' value='".h($Zh)."'>","</div></fieldset>\n";}function
selectActionPrint($x){echo"<fieldset><legend>".'Aktion'."</legend><div>","<input type='submit' value='".'Daten zeigen von'."'>"," <span id='noindex' title='".'VollstÃ¤ndige ÃœberprÃ¼fung der Tabelle'."'></span>","<script".nonce().">\n","var indexColumns = ";$e=array();foreach($x
...
messageQuery($G,$ai,$Lc=false){global$l;restart_session();$zd=&get_session("queries");if(!$zd[$_GET["db"]])$zd[$_GET["db"]]=array();if(strlen($G)>1e6)$G=preg_replace('~[\x80-\xFF]+$~','',substr($G,0,1e6))."\nâ€¦";$zd[$_GET["db"]][]=array($G,time(),$ai);$wh="sql-".count($zd[$_GET["db"]]);$I="<a href='#$wh' class='toggle'>".'SQL-Kommando'."</a>\n";if(!$Lc&&($Yi=$l->warnings())){$u="warnings-".count($zd[$_GET["db"]]);$I="<a href='#$u' class='toggle'>".'Warnungen'."</a>, $I<div id='$u' class='hidden'>\n$Yi</div>\n";}return" <span class='time'>".@date("H:i:s")."</span>"." $I<div id='$wh' class='hidden'><pre><code class='jush-".JUSH."'>".shorten_utf8($G,1000)."</code></pre>".($ai?" <span class='time'>($ai)</span>":'').(support("sql")?'<p><a href="'.h(str_replace("db=".urlencode(DB),"db=".urlencode($_GET["db"]),ME).'sql=&history='.(count($zd[$_GET["db"]])-1)).'">'.'Bearbeiten'.'</a>':'').'</div>';}function
...
as$Xf=>$X){if(!$Xf||preg_match("~$Xf~",$n["type"]))$I.="/$X";}}if($y&&!preg_match('~set|blob|bytea|raw|file|bool~',$n["type"]))$I.="/SQL";}if($n["auto_increment"]&&!$Fi)$I='Auto-Inkrement';return
explode("/",$I);}function
...
<p id="dbs">
';hidden_fields_get();$Jb=script("mixin(qsl('select'), {onmousedown: dbMouseDown, onchange: dbChange});");echo"<span title='".'Datenbank'."'>".'DB'.":</span> ".($i?html_select("db",array(""=>"")+$i,DB).$Jb:"<input name='db' value='".h(DB)."' autocapitalize='off' size='19'>\n"),"<input type='submit' value='".'AuswÃ¤hlen'."'".($i?" class='hidden'":"").">\n";if(support("scheme")){if($Pe!="db"&&DB!=""&&$f->select_db(DB)){echo"<br><span>".'Schema'.":</span> ".html_select("ns",array(""=>"")+$b->schemas(),$_GET["ns"]).$Jb;if($_GET["ns"]!="")set_schema($_GET["ns"]);}}foreach(array("import","sql","schema","dump","privileges")as$X){if(isset($_GET[$X])){echo"<input type='hidden' name='$X' value=''>";break;}}echo"</p></form>\n";}function
tablesPrint($S){echo"<ul id='tables'>".script("mixin(qs('#tables'), {onmouseover: menuOver, onmouseout: menuOut});");foreach($S
as$Q=>$O){$B=$this->tableName($O);if($B!="")echo'<li><a href="'.h(ME).'select='.urlencode($Q).'"'.bold($_GET["select"]==$Q||$_GET["edit"]==$Q,"select")." title='".'Daten auswÃ¤hlen'."'>".'zeigen'."</a> ",(support("table")||support("indexes")?'<a href="'.h(ME).'table='.urlencode($Q).'"'.bold(in_array($Q,array($_GET["table"],$_GET["create"],$_GET["indexes"],$_GET["foreign"],$_GET["trigger"])),(is_view($O)?"view":"structure"))." title='".'Struktur anzeigen'."'>$B</a>":"<span>$B</span>")."\n";}echo"</ul>\n";}}$b=(function_exists('adminer_object')?adminer_object():new
...
connect($M,$V,$E){if(ini_bool("mysql.allow_local_infile")){$this->error=sprintf('Deaktivieren Sie %s oder aktivieren Sie die Erweiterungen %s oder %s.',"'mysql.allow_local_infile'","MySQLi","PDO_MySQL");return
...
var offlineMessage = '".js_escape('Sie sind offline.')."';
...
check_invalid_login(){global$b;$Ud=array();foreach(glob(get_temp_dir()."/adminer.invalid*")as$p){$r=file_open_lock($p);if($r){$Ud=unserialize(stream_get_contents($r));file_unlock($r);break;}}$Td=($Ud?$Ud[$b->bruteForceKey()]:array());$Ze=($Td[1]>29?$Td[0]-time():0);if($Ze>0)auth_error(lang(array('Zu viele erfolglose Login-Versuche. Bitte probieren Sie es in %d Minute noch einmal.','Zu viele erfolglose Login-Versuche. Bitte probieren Sie es in %d Minuten noch einmal.'),ceil($Ze/60)));}$za=$_POST["auth"];if($za){session_regenerate_id();$Si=$za["driver"];$M=$za["server"];$V=$za["username"];$E=(string)$za["password"];$j=$za["db"];set_password($Si,$M,$V,$E);$_SESSION["db"][$Si][$M][$V][$j]=true;if($za["permanent"]){$y=implode("-",array_map('base64_encode',array($Si,$M,$V,$j)));$lg=$b->permanentLogin(true);$Zf[$y]="$y:".base64_encode($lg?encrypt_string($E,$lg):"");cookie("adminer_permanent",implode(" ",$Zf));}if(count($_POST)==1||DRIVER!=$Si||SERVER!=$M||$_GET["username"]!==$V||DB!=$j)redirect(auth_url($Si,$M,$V,$j));}elseif($_POST["logout"]&&(!$wd||verify_token())){foreach(array("pwds","db","dbs","queries")as$y)set_session($y,null);unset_permanent();redirect(substr(preg_replace('~\b(username|db|ns)=[^&]*&~','',ME),0,-1),'Abmeldung erfolgreich.'.' '.'Danke, dass Sie Adminer genutzt haben. <a href="https://www.adminer.org/de/donation/">Spenden willkommen!</a>.');}elseif($Zf&&!$_SESSION["pwds"]){session_regenerate_id();$lg=$b->permanentLogin();foreach($Zf
...
auth_error($m){global$b,$wd;$ih=session_name();if(isset($_GET["username"])){header("HTTP/1.1 403 Forbidden");if(($_COOKIE[$ih]||$_GET[$ih])&&!$wd)$m='Sitzungsdauer abgelaufen, bitte erneut anmelden.';else{restart_session();add_invalid_login();$E=get_password();if($E!==null){if($E===false)$m.=($m?'<br>':'').sprintf('Das Master-Passwort ist abgelaufen. <a href="https://www.adminer.org/de/extension/"%s>Implementieren</a> Sie die %s Methode, um es permanent zu machen.',target_blank(),'<code>permanentLogin()</code>');set_password(DRIVER,SERVER,$_GET["username"],null);}unset_permanent();}}if(!$_COOKIE[$ih]&&$_GET[$ih]&&ini_bool("session.use_only_cookies"))$m='UnterstÃ¼zung fÃ¼r PHP-Sessions muss aktiviert sein.';$Of=session_get_cookie_params();cookie("adminer_key",($_COOKIE["adminer_key"]?:rand_string()),$Of["lifetime"]);page_header('Login',$m,null);echo"<form action='' method='post'>\n","<div>";if(hidden_fields($_POST,array("auth")))echo"<p class='message'>".'Die Aktion wird nach erfolgreicher Anmeldung mit denselben Anmeldedaten ausgefÃ¼hrt.'."\n";echo"</div>\n";$b->loginForm();echo"</form>\n";page_footer("auth");exit;}if(isset($_GET["username"])&&!class_exists('Adminer\Db')){unset($_SESSION["pwds"][DRIVER]);unset_permanent();page_header('Keine Erweiterungen installiert',sprintf('Keine der unterstÃ¼tzten PHP-Erweiterungen (%s) ist vorhanden.',implode(", ",Driver::$fg)),false);page_footer("auth");exit;}stop_session(true);if(isset($_GET["username"])&&is_string(get_password())){list($Ad,$bg)=explode(":",SERVER,2);if(preg_match('~^\s*([-+]?\d+)~',$bg,$A)&&($A[1]<1024||$A[1]>65535))auth_error('Die Verbindung zu privilegierten Ports ist nicht erlaubt.');check_invalid_login();$f=connect($b->credentials());if(is_object($f)){$l=new
Driver($f);if($b->operators===null)$b->operators=$l->operators;if(isset($f->maria)||$f->cockroach)save_settings(array("vendor-".SERVER=>$Zb[DRIVER]));}}$ve=null;if(!is_object($f)||($ve=$b->login($_GET["username"],get_password()))!==true){$m=(is_string($f)?nl_br(h($f)):(is_string($ve)?$ve:'UngÃ¼ltige Anmelde-Informationen.'));auth_error($m.(preg_match('~^ | $~',get_password())?'<br>'.'Es gibt ein Leerzeichen im Eingabepasswort, das die Ursache sein kÃ¶nnte.':''));}if($_POST["logout"]&&$wd&&!verify_token()){page_header('Abmelden','CSRF Token ungÃ¼ltig. Bitte die Formulardaten erneut abschicken.');page_footer("db");exit;}if($za&&$_POST["token"])$_POST["token"]=$T;$m='';if($_POST){if(!verify_token()){$Od="max_input_vars";$Ge=ini_get($Od);if(extension_loaded("suhosin")){foreach(array("suhosin.request.max_vars","suhosin.post.max_vars")as$y){$X=ini_get($y);if($X&&(!$Ge||$X<$Ge)){$Od=$y;$Ge=$X;}}}$m=(!$_POST["token"]&&$Ge?sprintf('Die maximal erlaubte Anzahl der Felder ist Ã¼berschritten. Bitte %s erhÃ¶hen.',"'$Od'"):'CSRF Token ungÃ¼ltig. Bitte die Formulardaten erneut abschicken.'.' '.'Wenn Sie diese Anfrage nicht von Adminer gesendet haben, schlieÃŸen Sie diese Seite.');}}elseif($_SERVER["REQUEST_METHOD"]=="POST"){$m=sprintf('POST-Daten sind zu groÃŸ. Reduzieren Sie die GrÃ¶ÃŸe oder vergrÃ¶ÃŸern Sie den Wert %s in der Konfiguration.',"'post_max_size'");if(isset($_GET["sql"]))$m.=' '.'Sie kÃ¶nnen eine groÃŸe SQL-Datei per FTP hochladen und dann vom Server importieren.';}function
...
pack("x512");}}}$b->dumpFooter();exit;}page_header('Exportieren',$m,($_GET["export"]!=""?array("table"=>$_GET["export"]):array()),h(DB));echo'
<form action="" method="post">
<table class="layout">
';$Kb=array('','USE','DROP+CREATE','CREATE');$Ph=array('','DROP+CREATE','CREATE');$Hb=array('','TRUNCATE+INSERT','INSERT');if(JUSH=="sql")$Hb[]='INSERT+UPDATE';$J=get_settings("adminer_export");if(!$J)$J=array("output"=>"text","format"=>"sql","db_style"=>(DB!=""?"":"CREATE"),"table_style"=>"DROP+CREATE","data_style"=>"INSERT");if(!isset($J["events"])){$J["routines"]=$J["events"]=($_GET["dump"]=="");$J["triggers"]=$J["table_style"];}echo"<tr><th>".'Ergebnis'."<td>".html_radios("output",$b->dumpOutput(),$J["output"])."\n","<tr><th>".'Format'."<td>".html_radios("format",$b->dumpFormat(),$J["format"])."\n",(JUSH=="sqlite"?"":"<tr><th>".'Datenbank'."<td>".html_select('db_style',$Kb,$J["db_style"]).(support("type")?checkbox("types",1,$J["types"],'Benutzerdefinierte Typen'):"").(support("routine")?checkbox("routines",1,$J["routines"],'Routinen'):"").(support("event")?checkbox("events",1,$J["events"],'Ereignisse'):"")),"<tr><th>".'Tabellen'."<td>".html_select('table_style',$Ph,$J["table_style"]).checkbox("auto_increment",1,$J["auto_increment"],'Auto-Inkrement').(support("trigger")?checkbox("triggers",1,$J["triggers"],'Trigger'):""),"<tr><th>".'Daten'."<td>".html_select('data_style',$Hb,$J["data_style"]),'</table>
<p><input type="submit" value="Exportieren">
<input type="hidden" name="token" value="',$T,'">

<table>
',script("qsl('table').onclick = dumpClick;");$ig=array();if(DB!=""){$Wa=($a!=""?"":" checked");echo"<thead><tr>","<th style='text-align: left;'><label class='block'><input type='checkbox' id='check-tables'$Wa>".'Tabellen'."</label>".script("qs('#check-tables').onclick = partial(formCheck, /^tables\\[/);",""),"<th style='text-align: right;'><label class='block'>".'Daten'."<input type='checkbox' id='check-data'$Wa></label>".script("qs('#check-data').onclick = partial(formCheck, /^data\\[/);",""),"</thead>\n";$Vi="";$Qh=tables_list();foreach($Qh
...
';$Tc=true;foreach($ig
as$y=>$X){if($y!=""&&$X>1){echo($Tc?"<p>":" ")."<a href='".h(ME)."dump=".urlencode("$y%")."'>".h($y)."</a>";$Tc=false;}}}elseif(isset($_GET["privileges"])){page_header('Rechte');echo'<p class="links"><a href="'.h(ME).'user=">'.'Benutzer erstellen'."</a>";$H=$f->query("SELECT User, Host FROM mysql.".(DB==""?"user":"db WHERE ".q(DB)." LIKE Db")." ORDER BY Host, User");$ld=$H;if(!$H)$H=$f->query("SELECT SUBSTRING_INDEX(CURRENT_USER, '@', 1) AS User, SUBSTRING_INDEX(CURRENT_USER, '@', -1) AS Host");echo"<form action=''><p>\n";hidden_fields_get();echo"<input type='hidden' name='db' value='".h(DB)."'>\n",($ld?"":"<input type='hidden' name='grant' value=''>\n"),"<table class='odds'>\n","<thead><tr><th>".'Benutzer'."<th>".'Server'."<th></thead>\n";while($J=$H->fetch_assoc())echo'<tr><td>'.h($J["User"])."<td>".h($J["Host"]).'<td><a href="'.h(ME.'user='.urlencode($J["User"]).'&host='.urlencode($J["Host"])).'">'.'Bearbeiten'."</a>\n";if(!$ld||DB!="")echo"<tr><td><input name='user' autocapitalize='off'><td><input name='host' value='localhost' autocapitalize='off'><td><input type='submit' value='".'Bearbeiten'."'>\n";echo"</table>\n","</form>\n";}elseif(isset($_GET["sql"])){if(!$m&&$_POST["export"]){save_settings(array("output"=>$_POST["output"],"format"=>$_POST["format"]),"adminer_import");dump_headers("sql");$b->dumpTable("","");$b->dumpData("","table",$_POST["query"]);$b->dumpFooter();exit;}restart_session();$_d=&get_session("queries");$zd=&$_d[DB];if(!$m&&$_POST["clear"]){$zd=array();redirect(remove_from_uri("history"));}page_header((isset($_GET["import"])?'Importieren':'SQL-Kommando'),$m);if(!$m&&$_POST){$r=false;if(!isset($_GET["import"]))$G=$_POST["query"];elseif($_POST["webfile"]){$vh=$b->importServerPath();$r=@fopen((file_exists($vh)?$vh:"compress.zlib://$vh.gz"),"rb");$G=($r?fread($r,1e6):false);}else$G=get_file("sql_file",true,";");if(is_string($G)){if(function_exists('memory_get_usage')&&($Ie=ini_bytes("memory_limit"))!="-1")@ini_set("memory_limit",max($Ie,2*strlen($G)+memory_get_usage()+8e6));if($G!=""&&strlen($G)<1e6){$rg=$G.(preg_match("~;[ \t\r\n]*\$~",$G)?"":";");if(!$zd||reset(end($zd))!=$rg){restart_session();$zd[]=array($rg,time());set_session("queries",$_d);stop_session();}}$sh="(?:\\s|/\\*[\s\S]*?\\*/|(?:#|-- )[^\n]*\n?|--\r?\n)";$Qb=";";$C=0;$pc=true;$g=connect($b->credentials());if(is_object($g)&&DB!=""){$g->select_db(DB);if($_GET["ns"]!="")set_schema($_GET["ns"],$g);}$kb=0;$xc=array();$Pf='[\'"'.(JUSH=="sql"?'`#':(JUSH=="sqlite"?'`[':(JUSH=="mssql"?'[':''))).']|/\*|-- |$'.(JUSH=="pgsql"?'|\$[^$]*\$':'');$ki=microtime(true);$oa=get_settings("adminer_import");$gc=$b->dumpFormat();unset($gc["sql"]);while($G!=""){if(!$C&&preg_match("~^$sh*+DELIMITER\\s+(\\S+)~i",$G,$A)){$Qb=$A[1];$G=substr($G,strlen($A[0]));}else{preg_match('('.preg_quote($Qb)."\\s*|$Pf)",$G,$A,PREG_OFFSET_CAPTURE,$C);list($ed,$cg)=$A[0];if(!$ed&&$r&&!feof($r))$G.=fread($r,1e5);else{if(!$ed&&rtrim($G)=="")break;$C=$cg+strlen($ed);if($ed&&rtrim($ed)!=$Qb){$Pa=$l->hasCStyleEscapes()||(JUSH=="pgsql"&&($cg>0&&strtolower($G[$cg-1])=="e"));$Xf=($ed=='/*'?'\*/':($ed=='['?']':(preg_match('~^-- |^#~',$ed)?"\n":preg_quote($ed).($Pa?"|\\\\.":""))));while(preg_match("($Xf|\$)s",$G,$A,PREG_OFFSET_CAPTURE,$C)){$Qg=$A[0][0];if(!$Qg&&$r&&!feof($r))$G.=fread($r,1e5);else{$C=$A[0][1]+strlen($Qg);if(!$Qg||$Qg[0]!="\\")break;}}}else{$pc=false;$rg=substr($G,0,$cg);$kb++;$kg="<pre id='sql-$kb'><code class='jush-".JUSH."'>".$b->sqlCommandQuery($rg)."</code></pre>\n";if(JUSH=="sqlite"&&preg_match("~^$sh*+ATTACH\\b~i",$rg,$A)){echo$kg,"<p class='error'>".'ATTACH Abfragen werden nicht unterstÃ¼tzt.'."\n";$xc[]=" <a href='#sql-$kb'>$kb</a>";if($_POST["error_stops"])break;}else{if(!$_POST["only_errors"]){echo$kg;ob_flush();flush();}$_h=microtime(true);if($f->multi_query($rg)&&is_object($g)&&preg_match("~^$sh*+USE\\b~i",$rg))$g->query($rg);do{$H=$f->store_result();if($f->error){echo($_POST["only_errors"]?$kg:""),"<p class='error'>".'Fehler in der SQL-Abfrage'.($f->errno?" ($f->errno)":"").": ".error()."\n";$xc[]=" <a href='#sql-$kb'>$kb</a>";if($_POST["error_stops"])break
2;}else{$ai=" <span class='time'>(".format_time($_h).")</span>".(strlen($rg)<1000?" <a href='".h(ME)."sql=".urlencode(trim($rg))."'>".'Bearbeiten'."</a>":"");$qa=$f->affected_rows;$Yi=($_POST["only_errors"]?"":$l->warnings());$Zi="warnings-$kb";if($Yi)$ai.=", <a href='#$Zi'>".'Warnungen'."</a>".script("qsl('a').onclick = partial(toggle, '$Zi');","");$Ec=null;$Fc="explain-$kb";if(is_object($H)){$z=$_POST["limit"];$Af=select($H,$g,array(),$z);if(!$_POST["only_errors"]){echo"<form action='' method='post'>\n";$cf=$H->num_rows;echo"<p>".($cf?($z&&$cf>$z?sprintf('%d / ',$z):"").lang(array('%d Datensatz','%d DatensÃ¤tze'),$cf):""),$ai;if($g&&preg_match("~^($sh|\\()*+SELECT\\b~i",$rg)&&($Ec=explain($g,$rg)))echo", <a href='#$Fc'>Explain</a>".script("qsl('a').onclick = partial(toggle, '$Fc');","");$u="export-$kb";echo", <a href='#$u'>".'Exportieren'."</a>".script("qsl('a').onclick = partial(toggle, '$u');","")."<span id='$u' class='hidden'>: ".html_select("output",$b->dumpOutput(),$oa["output"])." ".html_select("format",$gc,$oa["format"])."<input type='hidden' name='query' value='".h($rg)."'>"." <input type='submit' name='export' value='".'Exportieren'."'><input type='hidden' name='token' value='$T'></span>\n"."</form>\n";}}else{if(preg_match("~^$sh*+(CREATE|DROP|ALTER)$sh++(DATABASE|SCHEMA)\\b~i",$rg)){restart_session();set_session("dbs",null);stop_session();}if(!$_POST["only_errors"])echo"<p class='message' title='".h($f->info)."'>".lang(array('Abfrage ausgefÃ¼hrt, %d Datensatz betroffen.','Abfrage ausgefÃ¼hrt, %d DatensÃ¤tze betroffen.'),$qa)."$ai\n";}echo($Yi?"<div id='$Zi' class='hidden'>\n$Yi</div>\n":"");if($Ec){echo"<div id='$Fc' class='hidden explain'>\n";select($Ec,$g,$Af);echo"</div>\n";}}$_h=microtime(true);}while($f->next_result());}$G=substr($G,$C);$C=0;}}}}if($pc)echo"<p class='message'>".'Kein Kommando vorhanden.'."\n";elseif($_POST["only_errors"])echo"<p class='message'>".lang(array('SQL-Abfrage erfolgreich ausgefÃ¼hrt.','%d SQL-Abfragen erfolgreich ausgefÃ¼hrt.'),$kb-count($xc))," <span class='time'>(".format_time($ki).")</span>\n";elseif($xc&&$kb>1)echo"<p class='error'>".'Fehler in der SQL-Abfrage'.": ".implode("",$xc)."\n";}else
echo"<p class='error'>".upload_error($G)."\n";}echo'
<form action="" method="post" enctype="multipart/form-data" id="form">
';$Cc="<input type='submit' value='".'AusfÃ¼hren'."' title='Ctrl+Enter'>";if(!isset($_GET["import"])){$rg=$_GET["sql"];if($_POST)$rg=$_POST["query"];elseif($_GET["history"]=="all")$rg=$zd;elseif($_GET["history"]!="")$rg=$zd[$_GET["history"]][0];echo"<p>";textarea("query",$rg,20);echo
...
as$B=>$n){if(!isset($n["privileges"][$Fi?"update":"insert"])||$b->fieldName($n)==""||$n["generated"])unset($o[$B]);}if($_POST&&!$m&&!isset($_GET["select"])){$ue=$_POST["referer"];if($_POST["insert"])$ue=($Fi?null:$_SERVER["REQUEST_URI"]);elseif(!preg_match('~^.+&select=.+$~',$ue))$ue=ME."select=".urlencode($a);$x=indexes($a);$Ai=unique_array($_GET["where"],$x);$ug="\nWHERE $Z";if(isset($_POST["delete"]))queries_redirect($ue,'Datensatz wurde gelÃ¶scht.',$l->delete($a,$ug,!$Ai));else{$N=array();foreach($o
as$B=>$n){$X=process_input($n);if($X!==false&&$X!==null)$N[idf_escape($B)]=$X;}if($Fi){if(!$N)redirect($ue);queries_redirect($ue,'Datensatz wurde geÃ¤ndert.',$l->update($a,$N,$ug,!$Ai));if(is_ajax()){page_headers();page_messages($m);exit;}}else{$H=$l->insert($a,$N);$le=($H?last_id():0);queries_redirect($ue,sprintf('Datensatz%s wurde eingefÃ¼gt.',($le?" $le":"")),$H);}}}$J=null;if($_POST["save"])$J=(array)$_POST["fields"];elseif($Z){$L=array();foreach($o
...
as$Lh=>$n)$cd[str_replace("`","``",$Lh)."`".str_replace("`","``",$n["field"])]=$Lh;$Df=array();$R=array();if($a!=""){$Df=fields($a);$R=table_status($a);if(!$R)$m='Keine Tabellen.';}$J=$_POST;$J["fields"]=(array)$J["fields"];if($J["auto_increment_col"])$J["fields"][$J["auto_increment_col"]]["auto_increment"]=true;if($_POST)save_settings(array("comments"=>$_POST["comments"],"defaults"=>$_POST["defaults"]));if($_POST&&!process_fields($J["fields"])&&!$m){if($_POST["drop"])queries_redirect(substr(ME,0,-1),'Tabelle wurde entfernt.',drop_tables(array($a)));else{$o=array();$ua=array();$Ji=false;$ad=array();$Cf=reset($Df);$sa=" FIRST";foreach($J["fields"]as$y=>$n){$q=$cd[$n["type"]];$vi=($q!==null?$_g[$q]:$n);if($n["field"]!=""){if(!$n["generated"])$n["default"]=null;$pg=process_field($n,$vi);$ua[]=array($n["orig"],$pg,$sa);if(!$Cf||$pg!==process_field($Cf,$Cf)){$o[]=array($n["orig"],$pg,$sa);if($n["orig"]!=""||$sa)$Ji=true;}if($q!==null)$ad[idf_escape($n["field"])]=($a!=""&&JUSH!="sqlite"?"ADD":" ").format_foreign_key(array('table'=>$cd[$n["type"]],'source'=>array($n["field"]),'target'=>array($vi["field"]),'on_delete'=>$n["on_delete"],));$sa=" AFTER ".idf_escape($n["field"]);}elseif($n["orig"]!=""){$Ji=true;$o[]=array($n["orig"]);}if($n["orig"]!=""){$Cf=next($Df);if(!$Cf)$sa="";}}$Tf="";if(support("partitioning")){if(isset($Rf[$J["partition_by"]])){$Of=array();foreach($J
as$y=>$X){if(preg_match('~^partition~',$y))$Of[$y]=$X;}foreach($Of["partition_names"]as$y=>$B){if($B==""){unset($Of["partition_names"][$y]);unset($Of["partition_values"][$y]);}}if($Of!=get_partitions_info($a)){$Uf=array();if($Of["partition_by"]=='RANGE'||$Of["partition_by"]=='LIST'){foreach($Of["partition_names"]as$y=>$B){$Y=$Of["partition_values"][$y];$Uf[]="\n  PARTITION ".idf_escape($B)." VALUES ".($Of["partition_by"]=='RANGE'?"LESS THAN":"IN").($Y!=""?" ($Y)":" MAXVALUE");}}$Tf.="\nPARTITION BY $Of[partition_by]($Of[partition])";if($Uf)$Tf.=" (".implode(",",$Uf)."\n)";elseif($Of["partitions"])$Tf.=" PARTITIONS ".(+$Of["partitions"]);}}elseif(preg_match("~partitioned~",$R["Create_options"]))$Tf.="\nREMOVE PARTITIONING";}$Je='Tabelle wurde geÃ¤ndert.';if($a==""){cookie("adminer_engine",$J["Engine"]);$Je='Tabelle wurde erstellt.';}$B=trim($J["name"]);queries_redirect(ME.(support("table")?"table=":"select=").urlencode($B),$Je,alter_table($a,$B,(JUSH=="sqlite"&&($Ji||$ad)?$ua:$o),$ad,($J["Comment"]!=$R["Comment"]?$J["Comment"]:null),($J["Engine"]&&$J["Engine"]!=$R["Engine"]?$J["Engine"]:""),($J["Collation"]&&$J["Collation"]!=$R["Collation"]?$J["Collation"]:""),($J["Auto_increment"]!=""?number($J["Auto_increment"]):""),$Tf));}}page_header(($a!=""?'Tabelle Ã¤ndern':'Tabelle erstellen'),$m,array("table"=>$a),h($a));if(!$_POST){$xi=$l->types();$J=array("Engine"=>$_COOKIE["adminer_engine"],"fields"=>array(array("field"=>"","type"=>(isset($xi["int"])?"int":(isset($xi["integer"])?"integer":"")),"on_update"=>"")),"partition_names"=>array(""),);if($a!=""){$J=$R;$J["name"]=$a;$J["fields"]=array();if(!$_GET["auto_increment"])$J["Auto_increment"]="";foreach($Df
...
<th><input type="submit" class="wayoff">','Spalte'.($re?"<span class='idxopts".($lh?"":" hidden")."'> (".'LÃ¤nge'.")</span>":"");if($re||support("descidx"))echo
checkbox("options",1,$lh,'Optionen',"indexOptionsShow(this.checked)","jsonly")."\n";echo'<th id="label-name">Name
<th><noscript>',"<input type='image' class='icon' name='add[0]' src='".h(preg_replace("~\\?.*~","",ME)."?file=plus.gif&version=5.0.6")."' alt='+' title='".'HinzufÃ¼gen'."'>",'</noscript>
</thead>
';if($F){echo"<tr><td>PRIMARY<td>";foreach($F["columns"]as$y=>$d)echo
select_input(" disabled",$o,$d),"<label><input disabled type='checkbox'>".'absteigend'."</label> ";echo"<td><td>\n";}$ae=1;foreach($J["indexes"]as$w){if(!$_POST["drop_col"]||$ae!=key($_POST["drop_col"])){echo"<tr><td>".html_select("indexes[$ae][type]",array(-1=>"")+$Kd,$w["type"],($ae==count($J["indexes"])?"indexesAddRow.call(this);":""),"label-type"),"<td>";ksort($w["columns"]);$t=1;foreach($w["columns"]as$y=>$d){echo"<span>".select_input(" name='indexes[$ae][columns][$t]' title='".'Spalte'."'",($o?array_combine($o,$o):$o),$d,"partial(".($t==count($w["columns"])?"indexesAddColumn":"indexesChangeColumn").", '".js_escape(JUSH=="sql"?"":$_GET["indexes"]."_")."')"),"<span class='idxopts".($lh?"":" hidden")."'>",($re?"<input type='number' name='indexes[$ae][lengths][$t]' class='size' value='".h($w["lengths"][$y])."' title='".'LÃ¤nge'."'>":""),(support("descidx")?checkbox("indexes[$ae][descs][$t]",1,$w["descs"][$y],'absteigend'):""),"</span> </span>";$t++;}echo"<td><input name='indexes[$ae][name]' value='".h($w["name"])."' autocapitalize='off' aria-labelledby='label-name'>\n","<td><input type='image' class='icon' name='drop_col[$ae]' src='".h(preg_replace("~\\?.*~","",ME)."?file=cross.gif&version=5.0.6")."' alt='x' title='".'Entfernen'."'>".script("qsl('input').onclick = partial(editingRemoveRow, 'indexes\$1[type]');");}$ae++;}echo'</table>
</div>
<p>
<input type="submit" value="Speichern">
<input type="hidden" name="token" value="',$T,'">
</form>
';}elseif(isset($_GET["database"])){$J=$_POST;if($_POST&&!$m&&!isset($_POST["add_x"])){$B=trim($J["name"]);if($_POST["drop"]){$_GET["db"]="";queries_redirect(remove_from_uri("db|database"),'Datenbank wurde entfernt.',drop_databases(array(DB)));}elseif(DB!==$B){if(DB!=""){$_GET["db"]=$B;queries_redirect(preg_replace('~\bdb=[^&]*&~','',ME)."db=".urlencode($B),'Datenbank wurde umbenannt.',rename_database($B,$J["collation"]));}else{$i=explode("\n",str_replace("\r","",$B));$Eh=true;$ke="";foreach($i
as$j){if(count($i)==1||$j!=""){if(!create_database($j,$J["collation"]))$Eh=false;$ke=$j;}}restart_session();set_session("dbs",null);queries_redirect(ME."db=".urlencode($ke),'Datenbank wurde erstellt.',$Eh);}}else{if(!$J["collation"])redirect(substr(ME,0,-1));query_redirect("ALTER DATABASE ".idf_escape($B).(preg_match('~^[a-z0-9_]+$~i',$J["collation"])?" COLLATE $J[collation]":""),substr(ME,0,-1),'Datenbank wurde geÃ¤ndert.');}}page_header(DB!=""?'Datenbank Ã¤ndern':'Datenbank erstellen',$m,array(),h(DB));$gb=collations();$B=DB;if($_POST)$B=$J["name"];elseif(DB!="")$J["collation"]=db_collation(DB,$gb);elseif(JUSH=="sql"){foreach(get_vals("SHOW GRANTS")as$ld){if(preg_match('~ ON (`(([^\\\\`]|``|\\\\.)*)%`\.\*)?~',$ld,$A)&&$A[1]){$B=stripcslashes(idf_unescape("`$A[2]`"));break;}}}echo'
<form action="" method="post">
<p>
',($_POST["add_x"]||strpos($B,"\n")?'<textarea autofocus name="name" rows="10" cols="40">'.h($B).'</textarea><br>':'<input name="name" autofocus value="'.h($B).'" data-maxlength="64" autocapitalize="off">')."\n".($gb?html_select("collation",array(""=>"(".'Kollation'.")")+$gb,$J["collation"]).doc_link(array('sql'=>"charset-charsets.html",'mariadb'=>"supported-character-sets-and-collations/",'mssql'=>"relational-databases/system-functions/sys-fn-helpcollations-transact-sql",)):""),'<input type="submit" value="Speichern">
';if(DB!="")echo"<input type='submit' name='drop' value='".'Entfernen'."'>".confirm(sprintf('%s entfernen?',DB))."\n";elseif(!$_POST["add_x"]&&$_GET["db"]=="")echo"<input type='image' class='icon' name='add' src='".h(preg_replace("~\\?.*~","",ME)."?file=plus.gif&version=5.0.6")."' alt='+' title='".'HinzufÃ¼gen'."'>\n";echo'<input type="hidden" name="token" value="',$T,'">
</form>
';}elseif(isset($_GET["scheme"])){$J=$_POST;if($_POST&&!$m){$_=preg_replace('~ns=[^&]*&~','',ME)."ns=";if($_POST["drop"])query_redirect("DROP SCHEMA ".idf_escape($_GET["ns"]),$_,'Schema wurde gelÃ¶scht.');else{$B=trim($J["name"]);$_.=urlencode($B);if($_GET["ns"]=="")query_redirect("CREATE SCHEMA ".idf_escape($B),$_,'Schema wurde erstellt.');elseif($_GET["ns"]!=$B)query_redirect("ALTER SCHEMA ".idf_escape($_GET["ns"])." RENAME TO ".idf_escape($B),$_,'Schema wurde geÃ¤ndert.');else
redirect($_);}}page_header($_GET["ns"]!=""?'Schema Ã¤ndern':'Schema erstellen',$m);if(!$J)$J["name"]=$_GET["ns"];echo'
<form action="" method="post">
<p><input name="name" autofocus value="',h($J["name"]),'" autocapitalize="off">
<input type="submit" value="Speichern">
';if($_GET["ns"]!="")echo"<input type='submit' name='drop' value='".'Entfernen'."'>".confirm(sprintf('%s entfernen?',$_GET["ns"]))."\n";echo'<input type="hidden" name="token" value="',$T,'">
</form>
';}elseif(isset($_GET["call"])){$da=($_GET["name"]?:$_GET["call"]);page_header('Aufrufen'.": ".h($da),$m);$Mg=routine($_GET["call"],(isset($_GET["callf"])?"FUNCTION":"PROCEDURE"));$Hd=array();$If=array();foreach($Mg["fields"]as$t=>$n){if(substr($n["inout"],-3)=="OUT")$If[$t]="@".idf_escape($n["field"])." AS ".idf_escape($n["field"]);if(!$n["inout"]||substr($n["inout"],0,2)=="IN")$Hd[]=$t;}if(!$m&&$_POST){$Qa=array();foreach($Mg["fields"]as$y=>$n){if(in_array($y,$Hd)){$X=process_input($n);if($X===false)$X="''";if(isset($If[$y]))$f->query("SET @".idf_escape($n["field"])." = $X");}$Qa[]=(isset($If[$y])?"@".idf_escape($n["field"]):$X);}$G=(isset($_GET["callf"])?"SELECT":"CALL")." ".table($da)."(".implode(", ",$Qa).")";$_h=microtime(true);$H=$f->multi_query($G);$qa=$f->affected_rows;echo$b->selectQuery($G,$_h,!$H);if(!$H)echo"<p class='error'>".error()."\n";else{$g=connect($b->credentials());if(is_object($g))$g->select_db(DB);do{$H=$f->store_result();if(is_object($H))select($H,$g);else
echo"<p class='message'>".lang(array('Routine wurde ausgefÃ¼hrt, %d Datensatz betroffen.','Routine wurde ausgefÃ¼hrt, %d DatensÃ¤tze betroffen.'),$qa)." <span class='time'>".@date("H:i:s")."</span>\n";}while($f->next_result());if($If)select($f->query("SELECT ".implode(", ",$If)));}}echo'
<form action="" method="post">
...
';}elseif(isset($_GET["foreign"])){$a=$_GET["foreign"];$B=$_GET["name"];$J=$_POST;if($_POST&&!$m&&!$_POST["add"]&&!$_POST["change"]&&!$_POST["change-js"]){if(!$_POST["drop"]){$J["source"]=array_filter($J["source"],'strlen');ksort($J["source"]);$Th=array();foreach($J["source"]as$y=>$X)$Th[$y]=$J["target"][$y];$J["target"]=$Th;}if(JUSH=="sqlite")$H=recreate_table($a,$a,array(),array(),array(" $B"=>($J["drop"]?"":" ".format_foreign_key($J))));else{$c="ALTER TABLE ".table($a);$H=($B==""||queries("$c DROP ".(JUSH=="sql"?"FOREIGN KEY ":"CONSTRAINT ").idf_escape($B)));if(!$J["drop"])$H=queries("$c ADD".format_foreign_key($J));}queries_redirect(ME."table=".urlencode($a),($J["drop"]?'FremdschlÃ¼ssel wurde entfernt.':($B!=""?'FremdschlÃ¼ssel wurde geÃ¤ndert.':'FremdschlÃ¼ssel wurde erstellt.')),$H);if(!$J["drop"])$m="$m<br>".'Quell- und Zielspalten mÃ¼ssen vom gleichen Datentyp sein, es muss unter den Zielspalten ein Index existieren und die referenzierten Daten mÃ¼ssen existieren.';}page_header('FremdschlÃ¼ssel',$m,array("table"=>$a),h($a));if($_POST){ksort($J["source"]);if($_POST["add"])$J["source"][]="";elseif($_POST["change"]||$_POST["change-js"])$J["target"]=array();}elseif($B!=""){$cd=foreign_keys($a);$J=$cd[$B];$J["source"][]="";}else{$J["table"]=$a;$J["source"]=array("");}echo'
<form action="" method="post">
';$rh=array_keys(fields($a));if($J["db"]!="")$f->select_db($J["db"]);if($J["ns"]!=""){$Ef=get_schema();set_schema($J["ns"]);}$zg=array_keys(array_filter(table_status('',true),'Adminer\fk_support'));$Th=array_keys(fields(in_array($J["table"],$zg)?$J["table"]:reset($zg)));$pf="this.form['change-js'].value = '1'; this.form.submit();";echo"<p>".'Zieltabelle'.": ".html_select("table",$zg,$J["table"],$pf)."\n";if(support("scheme")){$Tg=array_filter($b->schemas(),function($Sg){return!preg_match('~^information_schema$~i',$Sg);});echo'Schema'.": ".html_select("ns",$Tg,$J["ns"]!=""?$J["ns"]:$_GET["ns"],$pf);if($J["ns"]!="")set_schema($Ef);}elseif(JUSH!="sqlite"){$Lb=array();foreach($b->databases()as$j){if(!information_schema($j))$Lb[]=$j;}echo'DB'.": ".html_select("db",$Lb,$J["db"]!=""?$J["db"]:$_GET["db"],$pf);}echo'<input type="hidden" name="change-js" value="">
...
';if($B!="")echo'<input type="submit" name="drop" value="Entfernen">',confirm(sprintf('%s entfernen?',$B));echo'<input type="hidden" name="token" value="',$T,'">
</form>
';}elseif(isset($_GET["view"])){$a=$_GET["view"];$J=$_POST;$Ff="VIEW";if(JUSH=="pgsql"&&$a!=""){$O=table_status($a);$Ff=strtoupper($O["Engine"]);}if($_POST&&!$m){$B=trim($J["name"]);$wa=" AS\n$J[select]";$ue=ME."table=".urlencode($B);$Je='View wurde geÃ¤ndert.';$U=($_POST["materialized"]?"MATERIALIZED VIEW":"VIEW");if(!$_POST["drop"]&&$a==$B&&JUSH!="sqlite"&&$U=="VIEW"&&$Ff=="VIEW")query_redirect((JUSH=="mssql"?"ALTER":"CREATE OR REPLACE")." VIEW ".table($B).$wa,$ue,$Je);else{$Vh=$B."_adminer_".uniqid();drop_create("DROP $Ff ".table($a),"CREATE $U ".table($B).$wa,"DROP $U ".table($B),"CREATE $U ".table($Vh).$wa,"DROP $U ".table($Vh),($_POST["drop"]?substr(ME,0,-1):$ue),'View wurde entfernt.',$Je,'View wurde erstellt.',$a,$B);}}if(!$_POST&&$a!=""){$J=view($a);$J["name"]=$a;$J["materialized"]=($Ff!="VIEW");if(!$m)$m=error();}page_header(($a!=""?'View Ã¤ndern':'View erstellen'),$m,array("table"=>$a),h($a));echo'
<form action="" method="post">
<p>Name: <input name="name" value="',h($J["name"]),'" data-maxlength="64" autocapitalize="off">
',(support("materializedview")?" ".checkbox("materialized",1,$J["materialized"],'Strukturierte Ansicht'):""),'<p>';textarea("select",$J["select"]);echo'<p>
<input type="submit" value="Speichern">
';if($a!="")echo'<input type="submit" name="drop" value="Entfernen">',confirm(sprintf('%s entfernen?',$a));echo'<input type="hidden" name="token" value="',$T,'">
</form>
';}elseif(isset($_GET["event"])){$aa=$_GET["event"];$Sd=array("YEAR","QUARTER","MONTH","DAY","HOUR","MINUTE","WEEK","SECOND","YEAR_MONTH","DAY_HOUR","DAY_MINUTE","DAY_SECOND","HOUR_MINUTE","HOUR_SECOND","MINUTE_SECOND");$Ah=array("ENABLED"=>"ENABLE","DISABLED"=>"DISABLE","SLAVESIDE_DISABLED"=>"DISABLE ON SLAVE");$J=$_POST;if($_POST&&!$m){if($_POST["drop"])query_redirect("DROP EVENT ".idf_escape($aa),substr(ME,0,-1),'Ereignis wurde entfernt.');elseif(in_array($J["INTERVAL_FIELD"],$Sd)&&isset($Ah[$J["STATUS"]])){$Rg="\nON SCHEDULE ".($J["INTERVAL_VALUE"]?"EVERY ".q($J["INTERVAL_VALUE"])." $J[INTERVAL_FIELD]".($J["STARTS"]?" STARTS ".q($J["STARTS"]):"").($J["ENDS"]?" ENDS ".q($J["ENDS"]):""):"AT ".q($J["STARTS"]))." ON COMPLETION".($J["ON_COMPLETION"]?"":" NOT")." PRESERVE";queries_redirect(substr(ME,0,-1),($aa!=""?'Ereignis wurde geÃ¤ndert.':'Ereignis wurde erstellt.'),queries(($aa!=""?"ALTER EVENT ".idf_escape($aa).$Rg.($aa!=$J["EVENT_NAME"]?"\nRENAME TO ".idf_escape($J["EVENT_NAME"]):""):"CREATE EVENT ".idf_escape($J["EVENT_NAME"]).$Rg)."\n".$Ah[$J["STATUS"]]." COMMENT ".q($J["EVENT_COMMENT"]).rtrim(" DO\n$J[EVENT_DEFINITION]",";").";"));}}page_header(($aa!=""?'Ereignis Ã¤ndern'.": ".h($aa):'Ereignis erstellen'),$m);if(!$J&&$aa!=""){$K=get_rows("SELECT * FROM information_schema.EVENTS WHERE EVENT_SCHEMA = ".q(DB)." AND EVENT_NAME = ".q($aa));$J=reset($K);}echo'
<form action="" method="post">
<table class="layout">
...
<tr><th>Jede<td><input type="number" name="INTERVAL_VALUE" value="',h($J["INTERVAL_VALUE"]),'" class="size"> ',html_select("INTERVAL_FIELD",$Sd,$J["INTERVAL_FIELD"]),'<tr><th>Status<td>',html_select("STATUS",$Ah,$J["STATUS"]),'<tr><th>Kommentar<td><input name="EVENT_COMMENT" value="',h($J["EVENT_COMMENT"]),'" data-maxlength="64">
<tr><th><td>',checkbox("ON_COMPLETION","PRESERVE",$J["ON_COMPLETION"]=="PRESERVE",'Nach der AusfÃ¼hrung erhalten'),'</table>
<p>';textarea("EVENT_DEFINITION",$J["EVENT_DEFINITION"]);echo'<p>
<input type="submit" value="Speichern">
';if($aa!="")echo'<input type="submit" name="drop" value="Entfernen">',confirm(sprintf('%s entfernen?',$aa));echo'<input type="hidden" name="token" value="',$T,'">
</form>
';}elseif(isset($_GET["procedure"])){$da=($_GET["name"]?:$_GET["procedure"]);$Mg=(isset($_GET["function"])?"FUNCTION":"PROCEDURE");$J=$_POST;$J["fields"]=(array)$J["fields"];if($_POST&&!process_fields($J["fields"])&&!$m){$Bf=routine($_GET["procedure"],$Mg);$Vh="$J[name]_adminer_".uniqid();drop_create("DROP $Mg ".routine_id($da,$Bf),create_routine($Mg,$J),"DROP $Mg ".routine_id($J["name"],$J),create_routine($Mg,array("name"=>$Vh)+$J),"DROP $Mg ".routine_id($Vh,$J),substr(ME,0,-1),'Routine wurde entfernt.','Routine wurde geÃ¤ndert.','Routine wurde erstellt.',$da,$J["name"]);}page_header(($da!=""?(isset($_GET["function"])?'Funktion Ã¤ndern':'Prozedur Ã¤ndern').": ".h($da):(isset($_GET["function"])?'Funktion erstellen':'Prozedur erstellen')),$m);if(!$_POST&&$da!=""){$J=routine($_GET["procedure"],$Mg);$J["name"]=$da;}$gb=get_vals("SHOW CHARACTER SET");sort($gb);$Ng=routine_languages();echo($gb?"<datalist id='collations'>".optionlist($gb)."</datalist>":""),'
<form action="" method="post" id="form">
<p>Name: <input name="name" value="',h($J["name"]),'" data-maxlength="64" autocapitalize="off">
',($Ng?'Sprache'.": ".html_select("language",$Ng,$J["language"])."\n":""),'<input type="submit" value="Speichern">
<div class="scrollable">
<table class="nowrap">
';edit_fields($J["fields"],$gb,$Mg);if(isset($_GET["function"])){echo"<tr><td>".'Typ des RÃ¼ckgabewertes';edit_type("returns",$J["returns"],$gb,array(),(JUSH=="pgsql"?array("void","trigger"):array()));}echo'</table>
',script("editFields();"),'</div>
<p>';textarea("definition",$J["definition"]);echo'<p>
<input type="submit" value="Speichern">
';if($da!="")echo'<input type="submit" name="drop" value="Entfernen">',confirm(sprintf('%s entfernen?',$da));echo'<input type="hidden" name="token" value="',$T,'">
</form>
';}elseif(isset($_GET["sequence"])){$fa=$_GET["sequence"];$J=$_POST;if($_POST&&!$m){$_=substr(ME,0,-1);$B=trim($J["name"]);if($_POST["drop"])query_redirect("DROP SEQUENCE ".idf_escape($fa),$_,'Sequenz wurde gelÃ¶scht.');elseif($fa=="")query_redirect("CREATE SEQUENCE ".idf_escape($B),$_,'Sequenz wurde erstellt.');elseif($fa!=$B)query_redirect("ALTER SEQUENCE ".idf_escape($fa)." RENAME TO ".idf_escape($B),$_,'Sequenz wurde geÃ¤ndert.');else
redirect($_);}page_header($fa!=""?'Sequenz Ã¤ndern'.": ".h($fa):'Sequenz erstellen',$m);if(!$J)$J["name"]=$fa;echo'
<form action="" method="post">
<p><input name="name" value="',h($J["name"]),'" autocapitalize="off">
<input type="submit" value="Speichern">
';if($fa!="")echo"<input type='submit' name='drop' value='".'Entfernen'."'>".confirm(sprintf('%s entfernen?',$fa))."\n";echo'<input type="hidden" name="token" value="',$T,'">
</form>
';}elseif(isset($_GET["type"])){$ga=$_GET["type"];$J=$_POST;if($_POST&&!$m){$_=substr(ME,0,-1);if($_POST["drop"])query_redirect("DROP TYPE ".idf_escape($ga),$_,'Typ wurde gelÃ¶scht.');else
query_redirect("CREATE TYPE ".idf_escape(trim($J["name"]))." $J[as]",$_,'Typ wurde erstellt.');}page_header($ga!=""?'Typ Ã¤ndern'.": ".h($ga):'Typ erstellen',$m);if(!$J)$J["as"]="AS ";echo'
<form action="" method="post">
<p>
';if($ga!=""){$xi=$l->types();$vc=type_values($xi[$ga]);if($vc)echo"<code class='jush-".JUSH."'>ENUM (".h($vc).")</code>\n<p>";echo"<input type='submit' name='drop' value='".'Entfernen'."'>".confirm(sprintf('%s entfernen?',$ga))."\n";}else{echo'Name'.": <input name='name' value='".h($J['name'])."' autocapitalize='off'>\n",doc_link(array('pgsql'=>"datatype-enum.html",),"?");textarea("as",$J["as"]);echo"<p><input type='submit' value='".'Speichern'."'>\n";}echo'<input type="hidden" name="token" value="',$T,'">
</form>
';}elseif(isset($_GET["check"])){$a=$_GET["check"];$B=$_GET["name"];$J=$_POST;if($J&&!$m){if(JUSH=="sqlite")$H=recreate_table($a,$a,array(),array(),array(),0,array(),$B,($J["drop"]?"":$J["clause"]));else{$H=($B==""||queries("ALTER TABLE ".table($a)." DROP CONSTRAINT ".idf_escape($B)));if(!$J["drop"])$H=queries("ALTER TABLE ".table($a)." ADD".($J["name"]!=""?" CONSTRAINT ".idf_escape($J["name"]):"")." CHECK ($J[clause])");}queries_redirect(ME."table=".urlencode($a),($J["drop"]?'Check wurde abgebrochen.':($B!=""?'Check wurde geÃ¤ndert.':'Check wurde erstellt.')),$H);}page_header(($B!=""?'Check Ã¤ndern'.": ".h($B):'Check erstellen'),$m,array("table"=>$a));if(!$J){$Xa=$l->checkConstraints($a);$J=array("name"=>$B,"clause"=>$Xa[$B]);}echo'
<form action="" method="post">
...
';}elseif(isset($_GET["trigger"])){$a=$_GET["trigger"];$B=$_GET["name"];$ti=trigger_options();$J=(array)trigger($B,$a)+array("Trigger"=>$a."_bi");if($_POST){if(!$m&&in_array($_POST["Timing"],$ti["Timing"])&&in_array($_POST["Event"],$ti["Event"])&&in_array($_POST["Type"],$ti["Type"])){$mf=" ON ".table($a);$ac="DROP TRIGGER ".idf_escape($B).(JUSH=="pgsql"?$mf:"");$ue=ME."table=".urlencode($a);if($_POST["drop"])query_redirect($ac,$ue,'Trigger wurde entfernt.');else{if($B!="")queries($ac);queries_redirect($ue,($B!=""?'Trigger wurde geÃ¤ndert.':'Trigger wurde erstellt.'),queries(create_trigger($mf,$_POST)));if($B!="")queries(create_trigger($mf,$J+array("Type"=>reset($ti["Type"]))));}}$J=$_POST;}page_header(($B!=""?'Trigger Ã¤ndern'.": ".h($B):'Trigger erstellen'),$m,array("table"=>$a));echo'
<form action="" method="post" id="form">
<table class="layout">
<tr><th>Zeitpunkt<td>',html_select("Timing",$ti["Timing"],$J["Timing"],"triggerChange(/^".preg_quote($a,"/")."_[ba][iud]$/, '".js_escape($a)."', this.form);"),'<tr><th>Ereignis<td>',html_select("Event",$ti["Event"],$J["Event"],"this.form['Timing'].onchange();"),(in_array("UPDATE OF",$ti["Event"])?" <input name='Of' value='".h($J["Of"])."' class='hidden'>":""),'<tr><th>Typ<td>',html_select("Type",$ti["Type"],$J["Type"]),'</table>
...
as$ef=>$Jg){if(preg_match('~^(.+)(\(.*\))?$~U',$ef,$A))grant("REVOKE",array_keys($Jg),$A[2]," ON $A[1] FROM $Ye");}}}queries_redirect(ME."privileges=",(isset($_GET["host"])?'Benutzer wurde geÃ¤ndert.':'Benutzer wurde erstellt.'),!$m);if($_b)$f->query("DROP USER $Ye");}}page_header((isset($_GET["host"])?'Benutzer'.": ".h("$ha@$_GET[host]"):'Benutzer erstellen'),$m,array("privileges"=>array('','Rechte')));$J=$_POST;if($J)$md=$We;else{$J=$_GET+array("host"=>get_val("SELECT SUBSTRING_INDEX(CURRENT_USER, '@', -1)"));$J["pass"]=$kf;if($kf!="")$J["hashed"]=true;$md[(DB==""||$md?"":idf_escape(addcslashes(DB,"%_\\"))).".*"]=array();}echo'<form action="" method="post">
<table class="layout">
...
',"<table class='odds'>\n","<thead><tr><th colspan='2'>".'Rechte'.doc_link(array('sql'=>"grant.html#priv_level"));$t=0;foreach($md
as$ef=>$ld){echo'<th>'.($ef!="*.*"?"<input name='objects[$t]' value='".h($ef)."' size='10' autocapitalize='off'>":"<input type='hidden' name='objects[$t]' value='*.*' size='10'>*.*");$t++;}echo"</thead>\n";foreach(array(""=>"","Server Admin"=>'Server',"Databases"=>'Datenbank',"Tables"=>'Tabelle',"Columns"=>'Spalte',"Procedures"=>'Routine',)as$vb=>$Rb){foreach((array)$ng[$vb]as$mg=>$lb){echo"<tr><td".($Rb?">$Rb<td":" colspan='2'").' lang="en" title="'.h($lb).'">'.h($mg);$t=0;foreach($md
as$ef=>$ld){$B="'grants[$t][".h(strtoupper($mg))."]'";$Y=$ld[strtoupper($mg)];if($vb=="Server Admin"&&$ef!=(isset($md["*.*"])?"*.*":".*"))echo"<td>";elseif(isset($_GET["grant"]))echo"<td><select name=$B><option><option value='1'".($Y?" selected":"").">".'Erlauben'."<option value='0'".($Y=="0"?" selected":"").">".'Widerrufen'."</select>";else
...
<input type="submit" value="Speichern">
';if(isset($_GET["host"]))echo'<input type="submit" name="drop" value="Entfernen">',confirm(sprintf('%s entfernen?',"$ha@$_GET[host]"));echo'<input type="hidden" name="token" value="',$T,'">
</form>
';}elseif(isset($_GET["processlist"])){if(support("kill")){if($_POST&&!$m){$ge=0;foreach((array)$_POST["kill"]as$X){if(kill_process($X))$ge++;}queries_redirect(ME."processlist=",lang(array('%d Prozess gestoppt.','%d Prozesse gestoppt.'),$ge),$ge||!$_POST["kill"]);}}page_header('Prozessliste',$m);echo'
<form action="" method="post">
<div class="scrollable">
...
as$y=>$X)echo"<td>".((JUSH=="sql"&&$y=="Info"&&preg_match("~Query|Killed~",$J["Command"])&&$X!="")||(JUSH=="pgsql"&&$y=="current_query"&&$X!="<IDLE>")||(JUSH=="oracle"&&$y=="sql_text"&&$X!="")?"<code class='jush-".JUSH."'>".shorten_utf8($X,100,"</code>").' <a href="'.h(ME.($J["db"]!=""?"db=".urlencode($J["db"])."&":"")."sql=".urlencode($X)).'">'.'Klonen'.'</a>':h($X));echo"\n";}echo'</table>
</div>
<p>
...
as$ce=>$W)$_.=where_link($t++,$ce,$W);}$X=select_value($X,$_,$n,$Zh);$u=h("val[$Bi][".bracket_escape($y)."]");$Y=$_POST["val"][$Bi][bracket_escape($y)];$jc=!is_array($J[$y])&&is_utf8($X)&&$K[$Te][$y]==$J[$y]&&!$id[$y]&&!$n["generated"];$Xh=preg_match('~text|json|lob~',$n["type"]);echo"<td id='$u'".(preg_match(number_type(),$n["type"])&&is_numeric(strip_tags($X))?" class='number'":"");if(($_GET["modify"]&&$jc)||$Y!==null){$sd=h($Y!==null?$Y:$J[$y]);echo">".($Xh?"<textarea name='$u' cols='30' rows='".(substr_count($J[$y],"\n")+1)."'>$sd</textarea>":"<input name='$u' value='$sd' size='$re[$y]'>");}else{$we=strpos($X,"<i>â€¦</i>");echo" data-text='".($we?2:($Xh?1:0))."'".($jc?"":" data-warning='".h('Benutzen Sie den Link zum Bearbeiten dieses Wertes.')."'").">$X";}}}if($Ea)echo"<td>";$b->backwardKeysPrint($Ea,$K[$Te]);echo"</tr>\n";}if(is_ajax())exit;echo"</table>\n","</div>\n";}if(!is_ajax()){if($K||$D){$Bc=true;if($_GET["page"]!="last"){if($z==""||(count($K)<$z&&($K||!$D)))$fd=($D?$D*$z:0)+count($K);elseif(JUSH!="sql"||!$Wd){$fd=($Wd?false:found_rows($R,$Z));if($fd<max(1e4,2*($D+1)*$z))$fd=reset(slow_query(count_rows($a,$Z,$Wd,$nd)));else$Bc=false;}}$Mf=($z!=""&&($fd===false||$fd>$z||$D));if($Mf)echo(($fd===false?count($K)+1:$fd-$D*$z)>$z?'<p><a href="'.h(remove_from_uri("page")."&page=".($D+1)).'" class="loadmore">'.'Mehr Daten laden'.'</a>'.script("qsl('a').onclick = partial(selectLoadMore, ".(+$z).", '".'Lade'."â€¦');",""):''),"\n";}echo"<div class='footer'><div>\n";if($K||$D){if($Mf){$De=($fd===false?$D+(count($K)>=$z?2:1):floor(($fd-1)/$z));echo"<fieldset>";if(JUSH!="simpledb"){echo"<legend><a href='".h(remove_from_uri("page"))."'>".'Seite'."</a></legend>",script("qsl('a').onclick = function () { pageClick(this.href, +prompt('".'Seite'."', '".($D+1)."')); return false; };"),pagination(0,$D).($D>5?" â€¦":"");for($t=max(1,$D-4);$t<min($De,$D+5);$t++)echo
pagination($t,$D);if($De>0)echo($D+5<$De?" â€¦":""),($Bc&&$fd!==false?pagination($De,$D):" <a href='".h(remove_from_uri("page")."&page=last")."' title='~$De'>".'letzte'."</a>");}else
echo"<legend>".'Seite'."</legend>",pagination(0,$D).($D>1?" â€¦":""),($D?pagination($D,$D):""),($De>$D?pagination($D+1,$D).($De>$D+1?" â€¦":""):"");echo"</fieldset>\n";}echo"<fieldset>","<legend>".'Gesamtergebnis'."</legend>";$Xb=($Bc?"":"~ ").$fd;$qf="var checked = formChecked(this, /check/); selectCount('selected', this.checked ? '$Xb' : checked); selectCount('selected2', this.checked || !checked ? '$Xb' : checked);";echo
checkbox("all",1,0,($fd!==false?($Bc?"":"~ ").lang(array('%d Datensatz','%d DatensÃ¤tze'),$fd):""),$qf)."\n","</fieldset>\n";if($b->selectCommandPrint())echo'<fieldset',($_GET["modify"]?'':' class="jsonly"'),'><legend>Ã„ndern</legend><div>
...
';$dd=$b->dumpFormat();foreach((array)$_GET["columns"]as$d){if($d["fun"]){unset($dd['sql']);break;}}if($dd){print_fieldset("export",'Exportieren'." <span id='selected2'></span>");$Jf=$b->dumpOutput();echo($Jf?html_select("output",$Jf,$pa["output"])." ":""),html_select("format",$dd,$pa["format"])," <input type='submit' name='export' value='".'Exportieren'."'>\n","</div></fieldset>\n";}$b->selectEmailPrint(array_filter($oc,'strlen'),$e);}echo"</div></div>\n";if($b->selectImportPrint())echo"<div>","<a href='#import'>".'Importieren'."</a>",script("qsl('a').onclick = partial(toggle, 'import');",""),"<span id='import'".($_POST["import"]?"":" class='hidden'").">: ","<input type='file' name='csv_file'> ",html_select("separator",array("csv"=>"CSV,","csv;"=>"CSV;","tsv"=>"TSV"),$pa["format"])," <input type='submit' name='import' value='".'Importieren'."'>","</span>","</div>";echo"<input type='hidden' name='token' value='$T'>\n","</form>\n",(!$nd&&$L?"":script("tableCheck();"));}}}if(is_ajax()){ob_end_clean();exit;}}elseif(isset($_GET["variables"])){$O=isset($_GET["status"]);page_header($O?'Status':'Variablen');$Ri=($O?show_status():show_variables());if(!$Ri)echo"<p class='message'>".'Keine DatensÃ¤tze.'."\n";else{echo"<table>\n";foreach($Ri

...
as$C=>$o){$X=process_input($o);if($X!==false&&$X!==null)$O[idf_escape($C)]=$X;}if($_i){if(!$O)adm_redirect($A);queries_redirect($A,'Item has been updated.',$m->update($a,$O,$og,!$vi));if(is_ajax()){page_headers();page_messages($n);exit;}}else{$H=$m->insert($a,$O);$ie=($H?last_id():0);queries_redirect($A,sprintf('Item%s has been inserted.',($ie?" $ie":"")),$H);}}}$J=null;if($_POST["save"])$J=(array)$_POST["fields"];elseif($Z){$L=array();foreach($p

...
script("$ih.onclick = function () { return confirm('".($Oe?js_escape($Oe):'ä½ ç¢ºå®šå—ï¼Ÿ')."'); };","");}function
...
sprintf('%.3f ç§’',max(0,microtime(true)-$Kh));}function
relative_uri(){return
...
upload_error($m){$Ke=($m==UPLOAD_ERR_INI_SIZE?ini_get("upload_max_filesize"):0);return($m?'ç„¡æ³•ä¸Šå‚³æª”æ¡ˆã€‚'.($Ke?" ".sprintf('å…è¨±çš„æª”æ¡ˆä¸Šé™å¤§å°ç‚º %sBã€‚',$Ke):""):'æª”æ¡ˆä¸å­˜åœ¨ã€‚');}function
...
query($G,$Ji=false){$H=$this->pdo->query($G);$this->error="";if(!$H){list(,$this->errno,$this->error)=$this->pdo->errorInfo();if(!$this->error)$this->error='æœªçŸ¥éŒ¯èª¤ã€‚';return
...
connect($Fb){list(,,$E)=$Fb;if($E!="")return'è³‡æ–™åº«ä¸æ”¯æ´å¯†ç¢¼ã€‚';return
new
Db;}function
...
h($f->error);}function
check_sqlite_name($B){global$f;$Oc="db|sdb|sqlite";if(!preg_match("~^[^\\0]*\\.($Oc)\$~",$B)){$f->error=sprintf('è«‹ä½¿ç”¨ä¸‹åˆ—å…¶ä¸­ä¸€å€‹æ“´å……æ¨¡çµ„ %sã€‚',str_replace("|",", ",$Oc));return
false;}return
true;}function
create_database($j,$ib){global$f;if(file_exists($j)){$f->error='æª”æ¡ˆå·²å­˜åœ¨ã€‚';return
...
enumLength($n){$xc=$this->types['ä½¿ç”¨è€…é¡å‹'][$n["type"]];return($xc?type_values($xc):"");}function
setUserTypes($Ii){$this->types['ä½¿ç”¨è€…é¡å‹']=array_flip($Ii);}function
...
selectQuery($G,$Kh,$Rc=false){global$l;$I="</p>\n";if(!$Rc&&($kj=$l->warnings())){$u="warnings";$I=", <a href='#$u'>".'è­¦å‘Š'."</a>".script("qsl('a').onclick = partial(toggle, '$u');","")."$I<div id='$u' class='hidden'>\n$kj</div>\n";}return"<p><code class='jush-".JUSH."'>".h(str_replace("\n"," ",$G))."</code> <span class='time'>(".format_time($Kh).")</span>".(support("sql")?" <a href='".h(ME)."sql=".urlencode($G)."'>".'ç·¨è¼¯'."</a>":"").$I;}function
sqlCommandQuery($G){return
shorten_utf8(trim($G),1000);}function
rowDescription($Q){return"";}function
rowDescriptions($K,$gd){return$K;}function
selectLink($X,$n){}function
selectVal($X,$_,$n,$Mf){$I=($X===null?"<i>NULL</i>":(preg_match("~char|binary|boolean~",$n["type"])&&!preg_match("~var~",$n["type"])?"<code>$X</code>":$X));if(preg_match('~blob|bytea|raw|file~',$n["type"])&&!is_utf8($X))$I="<i>".sprintf('%d byte(s)',strlen($Mf))."</i>";if(preg_match('~json~',$n["type"]))$I="<code class='jush-js'>$I</code>";return($_?"<a href='".h($_)."'".(is_url($_)?target_blank():"").">$I</a>":$I);}function
...
as$n){echo"<tr><th>".h($n["field"]);$U=h($n["full_type"]);echo"<td><span title='".h($n["collation"])."'>".(in_array($U,(array)$Nh['ä½¿ç”¨è€…é¡å‹'])?"<a href='".h(ME.'type='.urlencode($U))."'>$U</a>":$U)."</span>",($n["null"]?" <i>NULL</i>":""),($n["auto_increment"]?" <i>".'è‡ªå‹•éå¢'."</i>":"");$k=h($n["default"]);echo(isset($n["default"])?" <span title='".'é è¨­å€¼'."'>[<b>".($n["generated"]?"<code class='jush-".JUSH."'>$k</code>":$k)."</b>]</span>":""),(support("comment")?"<td>".h($n["comment"]):""),"\n";}echo"</table>\n","</div>\n";}function
...
selectLengthPrint($ki){if($ki!==null){echo"<fieldset><legend>".'Text é•·åº¦'."</legend><div>","<input type='number' name='text_length' class='size' value='".h($ki)."'>","</div></fieldset>\n";}}function
...
messageQuery($G,$li,$Rc=false){global$l;restart_session();$Dd=&get_session("queries");if(!$Dd[$_GET["db"]])$Dd[$_GET["db"]]=array();if(strlen($G)>1e6)$G=preg_replace('~[\x80-\xFF]+$~','',substr($G,0,1e6))."\nâ€¦";$Dd[$_GET["db"]][]=array($G,time(),$li);$Gh="sql-".count($Dd[$_GET["db"]]);$I="<a href='#$Gh' class='toggle'>".'SQL å‘½ä»¤'."</a>\n";if(!$Rc&&($kj=$l->warnings())){$u="warnings-".count($Dd[$_GET["db"]]);$I="<a href='#$u' class='toggle'>".'è­¦å‘Š'."</a>, $I<div id='$u' class='hidden'>\n$kj</div>\n";}return" <span class='time'>".@date("H:i:s")."</span>"." $I<div id='$Gh' class='hidden'><pre><code class='jush-".JUSH."'>".shorten_utf8($G,1000)."</code></pre>".($li?" <span class='time'>($li)</span>":'').(support("sql")?'<p><a href="'.h(str_replace("db=".urlencode(DB),"db=".urlencode($_GET["db"]),ME).'sql=&history='.(count($Dd[$_GET["db"]])-1)).'">'.'ç·¨è¼¯'.'</a>':'').'</div>';}function
...
as$dg=>$X){if(!$dg||preg_match("~$dg~",$n["type"]))$I.="/$X";}}if($y&&!preg_match('~set|blob|bytea|raw|file|bool~',$n["type"]))$I.="/SQL";}if($n["auto_increment"]&&!$Ri)$I='è‡ªå‹•éå¢';return
explode("/",$I);}function
editInput($Q,$n,$Aa,$Y){if($n["type"]=="enum")return(isset($_GET["select"])?"<label><input type='radio'$Aa value='-1' checked><i>".'åŸå§‹'."</i></label> ":"").($n["null"]?"<label><input type='radio'$Aa value=''".($Y!==null||isset($_GET["select"])?"":" checked")."><i>NULL</i></label> ":"").enum_input("radio",$Aa,$n,$Y,$Y===0?0:null);return"";}function
...
';}$this->databasesPrint($Ue);$oa=array();if(DB==""||!$Ue){if(support("sql")){$oa[]="<a href='".h(ME)."sql='".bold(isset($_GET["sql"])&&!isset($_GET["import"])).">".'SQL å‘½ä»¤'."</a>";$oa[]="<a href='".h(ME)."import='".bold(isset($_GET["import"])).">".'åŒ¯å…¥'."</a>";}$oa[]="<a href='".h(ME)."dump=".urlencode(isset($_GET["table"])?$_GET["table"]:$_GET["select"])."' id='dump'".bold(isset($_GET["dump"])).">".'åŒ¯å‡º'."</a>";}$Nd=$_GET["ns"]!==""&&!$Ue&&DB!="";if($Nd)$oa[]='<a href="'.h(ME).'create="'.bold($_GET["create"]==="").">".'å»ºç«‹è³‡æ–™è¡¨'."</a>";echo($oa?"<p class='links'>\n".implode("\n",$oa)."\n":"");if($Nd){if($S)$this->tablesPrint($S);else
...
<p id="dbs">
';hidden_fields_get();$Ob=script("mixin(qsl('select'), {onmousedown: dbMouseDown, onchange: dbChange});");echo"<span title='".'è³‡æ–™åº«'."'>".'è³‡æ–™åº«'."</span>: ".($i?html_select("db",array(""=>"")+$i,DB).$Ob:"<input name='db' value='".h(DB)."' autocapitalize='off' size='19'>\n"),"<input type='submit' value='".'ä½¿ç”¨'."'".($i?" class='hidden'":"").">\n";if(support("scheme")){if($Ue!="db"&&DB!=""&&$f->select_db(DB)){echo"<br>".'è³‡æ–™è¡¨çµæ§‹'.": ".html_select("ns",array(""=>"")+$b->schemas(),$_GET["ns"]).$Ob;if($_GET["ns"]!="")set_schema($_GET["ns"]);}}foreach(array("import","sql","schema","dump","privileges")as$X){if(isset($_GET[$X])){echo"<input type='hidden' name='$X' value=''>";break;}}echo"</p></form>\n";}function
...
check_invalid_login(){global$b;$Zd=unserialize(@file_get_contents(get_temp_dir()."/adminer.invalid"));$Yd=($Zd?$Zd[$b->bruteForceKey()]:array());$ef=($Yd[1]>29?$Yd[0]-time():0);if($ef>0)auth_error(sprintf('ç™»éŒ„å¤±æ•—æ¬¡æ•¸éå¤šï¼Œè«‹ %d åˆ†é˜å¾Œé‡è©¦ã€‚',ceil($ef/60)));}$Ba=$_POST["auth"];if($Ba){session_regenerate_id();$ej=$Ba["driver"];$M=$Ba["server"];$V=$Ba["username"];$E=(string)$Ba["password"];$j=$Ba["db"];set_password($ej,$M,$V,$E);$_SESSION["db"][$ej][$M][$V][$j]=true;if($Ba["permanent"]){$y=base64_encode($ej)."-".base64_encode($M)."-".base64_encode($V)."-".base64_encode($j);$rg=$b->permanentLogin(true);$fg[$y]="$y:".base64_encode($rg?encrypt_string($E,$rg):"");cookie("adminer_permanent",implode(" ",$fg));}if(count($_POST)==1||DRIVER!=$ej||SERVER!=$M||$_GET["username"]!==$V||DB!=$j)redirect(auth_url($ej,$M,$V,$j));}elseif($_POST["logout"]&&(!$Ad||verify_token())){foreach(array("pwds","db","dbs","queries")as$y)set_session($y,null);unset_permanent();redirect(substr(preg_replace('~\b(username|db|ns)=[^&]*&~','',ME),0,-1),'æˆåŠŸç™»å‡ºã€‚'.' '.'æ„Ÿè¬ä½¿ç”¨Adminerï¼Œè«‹è€ƒæ…®ç‚ºæˆ‘å€‘<a href="https://www.adminer.org/en/donation/">ææ¬¾ï¼ˆè‹±æ–‡ç¶²é ï¼‰</a>ã€‚');}elseif($fg&&!$_SESSION["pwds"]){session_regenerate_id();$rg=$b->permanentLogin();foreach($fg
...
auth_error($m){global$b,$Ad;$rh=session_name();if(isset($_GET["username"])){header("HTTP/1.1 403 Forbidden");if(($_COOKIE[$rh]||$_GET[$rh])&&!$Ad)$m='Session å·²éæœŸï¼Œè«‹é‡æ–°ç™»å…¥ã€‚';else{restart_session();add_invalid_login();$E=get_password();if($E!==null){if($E===false)$m.=($m?'<br>':'').sprintf('ä¸»å¯†ç¢¼å·²éæœŸã€‚<a href="https://www.adminer.org/en/extension/"%s>è«‹æ“´å±•</a> %s æ–¹æ³•è®“å®ƒæ°¸ä¹…åŒ–ã€‚',target_blank(),'<code>permanentLogin()</code>');set_password(DRIVER,SERVER,$_GET["username"],null);}unset_permanent();}}if(!$_COOKIE[$rh]&&$_GET[$rh]&&ini_bool("session.use_only_cookies"))$m='Session å¿…é ˆè¢«å•Ÿç”¨ã€‚';$Uf=session_get_cookie_params();cookie("adminer_key",($_COOKIE["adminer_key"]?:rand_string()),$Uf["lifetime"]);page_header('ç™»å…¥',$m,null);echo"<form action='' method='post'>\n","<div>";if(hidden_fields($_POST,array("auth")))echo"<p class='message'>".'æ­¤æ“ä½œå°‡åœ¨æˆåŠŸä½¿ç”¨ç›¸åŒçš„æ†‘æ“šç™»éŒ„å¾ŒåŸ·è¡Œã€‚'."\n";echo"</div>\n";$b->loginForm();echo"</form>\n";page_footer("auth");exit;}if(isset($_GET["username"])&&!class_exists('Adminer\Db')){unset($_SESSION["pwds"][DRIVER]);unset_permanent();page_header('ç„¡æ“´å……æ¨¡çµ„',sprintf('æ²’æœ‰ä»»ä½•æ”¯æ´çš„ PHP æ“´å……æ¨¡çµ„ï¼ˆ%sï¼‰ã€‚',implode(", ",Driver::$lg)),false);page_footer("auth");exit;}stop_session(true);if(isset($_GET["username"])&&is_string(get_password())){list($Fd,$hg)=explode(":",SERVER,2);if(preg_match('~^\s*([-+]?\d+)~',$hg,$A)&&($A[1]<1024||$A[1]>65535))auth_error('ä¸å…è¨±é€£æ¥åˆ°ç‰¹æ¬ŠåŸ ã€‚');check_invalid_login();$f=connect($b->credentials());if(is_object($f)){$l=new
Driver($f);if($b->operators===null)$b->operators=$l->operators;}}$Ae=null;if(!is_object($f)||($Ae=$b->login($_GET["username"],get_password()))!==true){$m=(is_string($f)?nl_br(h($f)):(is_string($Ae)?$Ae:'ç„¡æ•ˆçš„æ†‘è­‰ã€‚'));auth_error($m.(preg_match('~^ | $~',get_password())?'<br>'.'æ‚¨è¼¸å…¥çš„å¯†ç¢¼ä¸­æœ‰ä¸€å€‹ç©ºæ ¼ï¼Œé€™å¯èƒ½æ˜¯å°è‡´å•é¡Œçš„åŸå› ã€‚':''));}if($_POST["logout"]&&$Ad&&!verify_token()){page_header('ç™»å‡º','ç„¡æ•ˆçš„ CSRF tokenã€‚è«‹é‡æ–°ç™¼é€è¡¨å–®ã€‚');page_footer("db");exit;}if($Ba&&$_POST["token"])$_POST["token"]=$T;$m='';if($_POST){if(!verify_token()){$Td="max_input_vars";$Le=ini_get($Td);if(extension_loaded("suhosin")){foreach(array("suhosin.request.max_vars","suhosin.post.max_vars")as$y){$X=ini_get($y);if($X&&(!$Le||$X<$Le)){$Td=$y;$Le=$X;}}}$m=(!$_POST["token"]&&$Le?sprintf('è¶…éå…è¨±çš„å­—æ®µæ•¸é‡çš„æœ€å¤§å€¼ã€‚è«‹å¢åŠ  %sã€‚',"'$Td'"):'ç„¡æ•ˆçš„ CSRF tokenã€‚è«‹é‡æ–°ç™¼é€è¡¨å–®ã€‚'.' '.'å¦‚æœæ‚¨ä¸¦æ²’æœ‰å¾Adminerç™¼é€è«‹æ±‚ï¼Œè«‹é—œé–‰æ­¤é é¢ã€‚');}}elseif($_SERVER["REQUEST_METHOD"]=="POST"){$m=sprintf('POST è³‡æ–™å¤ªå¤§ã€‚æ¸›å°‘è³‡æ–™æˆ–è€…å¢åŠ  %s çš„è¨­å®šå€¼ã€‚',"'post_max_size'");if(isset($_GET["sql"]))$m.=' '.'æ‚¨å¯ä»¥é€šéFTPä¸Šå‚³å¤§å‹SQLæª”ä¸¦å¾ä¼ºæœå™¨å°å…¥ã€‚';}function
...
as$y=>$X){$_="";if(isset($ze[$y])&&!$e[$ze[$y]]){if($Gf&&JUSH=="sql"){$Q=$J[array_search("table=",$ze)];$_=ME.$ze[$y].urlencode($Gf[$Q]!=""?$Gf[$Q]:$Q);}else{$_=ME."edit=".urlencode($ze[$y]);foreach($x[$ze[$y]]as$gb=>$ge)$_.="&where".urlencode("[".bracket_escape($gb)."]")."=".urlencode($J[$ge]);}}elseif(is_url($X))$_=$X;if($X===null)$X="<i>NULL</i>";elseif($La[$y]&&!is_utf8($X))$X="<i>".sprintf('%d byte(s)',strlen($X))."</i>";else{$X=h($X);if($Ii[$y]==254)$X="<code>$X</code>";}if($_)$X="<a href='".h($_)."'".(is_url($_)?target_blank():'').">$X</a>";echo"<td>$X";}}echo($t?"</table>\n</div>":"<p class='message'>".'æ²’æœ‰è³‡æ–™è¡Œã€‚')."\n";return$I;}function
...
<td><input type="radio" name="auto_increment_col" value=""><abbr id="label-ai" title="è‡ªå‹•éå¢">AI</abbr>',doc_link(array('sql'=>"example-auto-increment.html",'mariadb'=>"auto_increment/",'sqlite'=>"autoinc.html",'pgsql'=>"datatype-numeric.html#DATATYPE-SERIAL",'mssql'=>"t-sql/statements/create-table-transact-sql-identity-property",)),'<td id="label-default"',$Sb,'>é è¨­å€¼
',(support("comment")?"<td id='label-comment'$qb>".'è¨»è§£':"");}echo'<td>',"<input type='image' class='icon' name='add[".(support("move_col")?0:count($o))."]' src='".h(preg_replace("~\\?.*~","",ME)."?file=plus.gif&version=5.0.4")."' alt='+' title='".'æ–°å¢ä¸‹ä¸€ç­†'."'>".script("row_count = ".count($o).";"),'</thead>
<tbody>
...
';}echo'<input type="hidden" name="fields[',$t,'][orig]" value="',h($Hf),'">';edit_type("fields[$t]",$n,$jb,$hd);if($U=="TABLE"){echo'<td>',checkbox("fields[$t][null]",1,$n["null"],"","","block","label-null"),'<td><label class="block"><input type="radio" name="auto_increment_col" value="',$t,'"',($n["auto_increment"]?" checked":""),' aria-labelledby="label-ai"></label><td',$Sb,'>',($l->generated?html_select("fields[$t][generated]",array_merge(array("","DEFAULT"),$l->generated),$n["generated"])." ":checkbox("fields[$t][generated]",1,$n["generated"],"","","","label-default")),'<input name="fields[',$t,'][default]" value="',h($n["default"]),'" aria-labelledby="label-default">',(support("comment")?"<td$qb><input name='fields[$t][comment]' value='".h($n["comment"])."' data-maxlength='".(min_version(5.5)?1024:255)."' aria-labelledby='label-comment'>":"");}echo"<td>",(support("move_col")?"<input type='image' class='icon' name='add[$t]' src='".h(preg_replace("~\\?.*~","",ME)."?file=plus.gif&version=5.0.4")."' alt='+' title='".'æ–°å¢ä¸‹ä¸€ç­†'."'> "."<input type='image' class='icon' name='up[$t]' src='".h(preg_replace("~\\?.*~","",ME)."?file=up.gif&version=5.0.4")."' alt='â†‘' title='".'ä¸Šç§»'."'> "."<input type='image' class='icon' name='down[$t]' src='".h(preg_replace("~\\?.*~","",ME)."?file=down.gif&version=5.0.4")."' alt='â†“' title='".'ä¸‹ç§»'."'> ":""),($Hf==""||support("drop_col")?"<input type='image' class='icon' name='drop_col[$t]' src='".h(preg_replace("~\\?.*~","",ME)."?file=cross.gif&version=5.0.4")."' alt='x' title='".'ç§»é™¤'."'>":"");}}function
...
as$y=>$X)echo"<tr valign='top'><td>".h($X[0])."<td>".h($X[1])."<th>".h($y)."<td><a href='".h(ME.'trigger='.urlencode($a).'&name='.urlencode($y))."'>".'ä¿®æ”¹'."</a>\n";echo"</table>\n";}echo'<p class="links"><a href="'.h(ME).'trigger='.urlencode($a).'">'.'å»ºç«‹è§¸ç™¼å™¨'."</a>\n";}}elseif(isset($_GET["schema"])){page_header('è³‡æ–™åº«çµæ§‹',"",array(),h(DB.($_GET["ns"]?".$_GET[ns]":"")));$Yh=array();$Zh=array();$ea=($_GET["schema"]?:$_COOKIE["adminer_schema-".str_replace(".","_",DB)]);preg_match_all('~([^:]+):([-0-9.]+)x([-0-9.]+)(_|$)~',$ea,$Fe,PREG_SET_ORDER);foreach($Fe
...
pack("x512");}}}$b->dumpFooter();exit;}page_header('åŒ¯å‡º',$m,($_GET["export"]!=""?array("table"=>$_GET["export"]):array()),h(DB));echo'
<form action="" method="post">
<table class="layout">
';$Pb=array('','USE','DROP+CREATE','CREATE');$ai=array('','DROP+CREATE','CREATE');$Lb=array('','TRUNCATE+INSERT','INSERT');if(JUSH=="sql")$Lb[]='INSERT+UPDATE';parse_str($_COOKIE["adminer_export"],$J);if(!$J)$J=array("output"=>"text","format"=>"sql","db_style"=>(DB!=""?"":"CREATE"),"table_style"=>"DROP+CREATE","data_style"=>"INSERT");if(!isset($J["events"])){$J["routines"]=$J["events"]=($_GET["dump"]=="");$J["triggers"]=$J["table_style"];}echo"<tr><th>".'è¼¸å‡º'."<td>".html_radios("output",$b->dumpOutput(),$J["output"])."\n","<tr><th>".'æ ¼å¼'."<td>".html_radios("format",$b->dumpFormat(),$J["format"])."\n",(JUSH=="sqlite"?"":"<tr><th>".'è³‡æ–™åº«'."<td>".html_select('db_style',$Pb,$J["db_style"]).(support("type")?checkbox("types",1,$J["types"],'ä½¿ç”¨è€…é¡å‹'):"").(support("routine")?checkbox("routines",1,$J["routines"],'ç¨‹åº'):"").(support("event")?checkbox("events",1,$J["events"],'äº‹ä»¶'):"")),"<tr><th>".'è³‡æ–™è¡¨'."<td>".html_select('table_style',$ai,$J["table_style"]).checkbox("auto_increment",1,$J["auto_increment"],'è‡ªå‹•éå¢').(support("trigger")?checkbox("triggers",1,$J["triggers"],'è§¸ç™¼å™¨'):""),"<tr><th>".'è³‡æ–™'."<td>".html_select('data_style',$Lb,$J["data_style"]),'</table>
<p><input type="submit" value="åŒ¯å‡º">
<input type="hidden" name="token" value="',$T,'">

<table>
',script("qsl('table').onclick = dumpClick;");$og=array();if(DB!=""){$Za=($a!=""?"":" checked");echo"<thead><tr>","<th style='text-align: left;'><label class='block'><input type='checkbox' id='check-tables'$Za>".'è³‡æ–™è¡¨'."</label>".script("qs('#check-tables').onclick = partial(formCheck, /^tables\\[/);",""),"<th style='text-align: right;'><label class='block'>".'è³‡æ–™'."<input type='checkbox' id='check-data'$Za></label>".script("qs('#check-data').onclick = partial(formCheck, /^data\\[/);",""),"</thead>\n";$hj="";$bi=tables_list();foreach($bi
...
';$Yc=true;foreach($og
as$y=>$X){if($y!=""&&$X>1){echo($Yc?"<p>":" ")."<a href='".h(ME)."dump=".urlencode("$y%")."'>".h($y)."</a>";$Yc=false;}}}elseif(isset($_GET["privileges"])){page_header('æ¬Šé™');echo'<p class="links"><a href="'.h(ME).'user=">'.'å»ºç«‹ä½¿ç”¨è€…'."</a>";$H=$f->query("SELECT User, Host FROM mysql.".(DB==""?"user":"db WHERE ".q(DB)." LIKE Db")." ORDER BY Host, User");$qd=$H;if(!$H)$H=$f->query("SELECT SUBSTRING_INDEX(CURRENT_USER, '@', 1) AS User, SUBSTRING_INDEX(CURRENT_USER, '@', -1) AS Host");echo"<form action=''><p>\n";hidden_fields_get();echo"<input type='hidden' name='db' value='".h(DB)."'>\n",($qd?"":"<input type='hidden' name='grant' value=''>\n"),"<table class='odds'>\n","<thead><tr><th>".'å¸³è™Ÿ'."<th>".'ä¼ºæœå™¨'."<th></thead>\n";while($J=$H->fetch_assoc())echo'<tr><td>'.h($J["User"])."<td>".h($J["Host"]).'<td><a href="'.h(ME.'user='.urlencode($J["User"]).'&host='.urlencode($J["Host"])).'">'.'ç·¨è¼¯'."</a>\n";if(!$qd||DB!="")echo"<tr><td><input name='user' autocapitalize='off'><td><input name='host' value='localhost' autocapitalize='off'><td><input type='submit' value='".'ç·¨è¼¯'."'>\n";echo"</table>\n","</form>\n";}elseif(isset($_GET["sql"])){if(!$m&&$_POST["export"]){dump_headers("sql");$b->dumpTable("","");$b->dumpData("","table",$_POST["query"]);$b->dumpFooter();exit;}restart_session();$Ed=&get_session("queries");$Dd=&$Ed[DB];if(!$m&&$_POST["clear"]){$Dd=array();redirect(remove_from_uri("history"));}page_header((isset($_GET["import"])?'åŒ¯å…¥':'SQL å‘½ä»¤'),$m);if(!$m&&$_POST){$r=false;if(!isset($_GET["import"]))$G=$_POST["query"];elseif($_POST["webfile"]){$Fh=$b->importServerPath();$r=@fopen((file_exists($Fh)?$Fh:"compress.zlib://$Fh.gz"),"rb");$G=($r?fread($r,1e6):false);}else$G=get_file("sql_file",true,";");if(is_string($G)){if(function_exists('memory_get_usage')&&($Ne=ini_bytes("memory_limit"))!="-1")@ini_set("memory_limit",max($Ne,2*strlen($G)+memory_get_usage()+8e6));if($G!=""&&strlen($G)<1e6){$xg=$G.(preg_match("~;[ \t\r\n]*\$~",$G)?"":";");if(!$Dd||reset(end($Dd))!=$xg){restart_session();$Dd[]=array($xg,time());set_session("queries",$Ed);stop_session();}}$Ch="(?:\\s|/\\*[\s\S]*?\\*/|(?:#|-- )[^\n]*\n?|--\r?\n)";$Vb=";";$C=0;$uc=true;$g=connect($b->credentials());if(is_object($g)&&DB!=""){$g->select_db(DB);if($_GET["ns"]!="")set_schema($_GET["ns"],$g);}$ob=0;$Bc=array();$Vf='[\'"'.(JUSH=="sql"?'`#':(JUSH=="sqlite"?'`[':(JUSH=="mssql"?'[':''))).']|/\*|-- |$'.(JUSH=="pgsql"?'|\$[^$]*\$':'');$wi=microtime(true);parse_str($_COOKIE["adminer_export"],$ra);$lc=$b->dumpFormat();unset($lc["sql"]);while($G!=""){if(!$C&&preg_match("~^$Ch*+DELIMITER\\s+(\\S+)~i",$G,$A)){$Vb=$A[1];$G=substr($G,strlen($A[0]));}else{preg_match('('.preg_quote($Vb)."\\s*|$Vf)",$G,$A,PREG_OFFSET_CAPTURE,$C);list($jd,$ig)=$A[0];if(!$jd&&$r&&!feof($r))$G.=fread($r,1e5);else{if(!$jd&&rtrim($G)=="")break;$C=$ig+strlen($jd);if($jd&&rtrim($jd)!=$Vb){$Sa=$l->hasCStyleEscapes()||(JUSH=="pgsql"&&($ig>0&&strtolower($G[$ig-1])=="e"));$dg=($jd=='/*'?'\*/':($jd=='['?']':(preg_match('~^-- |^#~',$jd)?"\n":preg_quote($jd).($Sa?"|\\\\.":""))));while(preg_match("($dg|\$)s",$G,$A,PREG_OFFSET_CAPTURE,$C)){$Zg=$A[0][0];if(!$Zg&&$r&&!feof($r))$G.=fread($r,1e5);else{$C=$A[0][1]+strlen($Zg);if(!$Zg||$Zg[0]!="\\")break;}}}else{$uc=false;$xg=substr($G,0,$ig);$ob++;$qg="<pre id='sql-$ob'><code class='jush-".JUSH."'>".$b->sqlCommandQuery($xg)."</code></pre>\n";if(JUSH=="sqlite"&&preg_match("~^$Ch*+ATTACH\\b~i",$xg,$A)){echo$qg,"<p class='error'>".'ä¸æ”¯æ´ATTACHæŸ¥è©¢ã€‚'."\n";$Bc[]=" <a href='#sql-$ob'>$ob</a>";if($_POST["error_stops"])break;}else{if(!$_POST["only_errors"]){echo$qg;ob_flush();flush();}$Kh=microtime(true);if($f->multi_query($xg)&&is_object($g)&&preg_match("~^$Ch*+USE\\b~i",$xg))$g->query($xg);do{$H=$f->store_result();if($f->error){echo($_POST["only_errors"]?$qg:""),"<p class='error'>".'æŸ¥è©¢ç™¼ç”ŸéŒ¯èª¤'.($f->errno?" ($f->errno)":"").": ".error()."\n";$Bc[]=" <a href='#sql-$ob'>$ob</a>";if($_POST["error_stops"])break
2;}else{$li=" <span class='time'>(".format_time($Kh).")</span>".(strlen($xg)<1000?" <a href='".h(ME)."sql=".urlencode(trim($xg))."'>".'ç·¨è¼¯'."</a>":"");$ta=$f->affected_rows;$kj=($_POST["only_errors"]?"":$l->warnings());$lj="warnings-$ob";if($kj)$li.=", <a href='#$lj'>".'è­¦å‘Š'."</a>".script("qsl('a').onclick = partial(toggle, '$lj');","");$Jc=null;$Kc="explain-$ob";if(is_object($H)){$z=$_POST["limit"];$Gf=select($H,$g,array(),$z);if(!$_POST["only_errors"]){echo"<form action='' method='post'>\n";$hf=$H->num_rows;echo"<p>".($hf?($z&&$hf>$z?sprintf('%d / ',$z):"").sprintf('%d è¡Œ',$hf):""),$li;if($g&&preg_match("~^($Ch|\\()*+SELECT\\b~i",$xg)&&($Jc=explain($g,$xg)))echo", <a href='#$Kc'>Explain</a>".script("qsl('a').onclick = partial(toggle, '$Kc');","");$u="export-$ob";echo", <a href='#$u'>".'åŒ¯å‡º'."</a>".script("qsl('a').onclick = partial(toggle, '$u');","")."<span id='$u' class='hidden'>: ".html_select("output",$b->dumpOutput(),$ra["output"])." ".html_select("format",$lc,$ra["format"])."<input type='hidden' name='query' value='".h($xg)."'>"." <input type='submit' name='export' value='".'åŒ¯å‡º'."'><input type='hidden' name='token' value='$T'></span>\n"."</form>\n";}}else{if(preg_match("~^$Ch*+(CREATE|DROP|ALTER)$Ch++(DATABASE|SCHEMA)\\b~i",$xg)){restart_session();set_session("dbs",null);stop_session();}if(!$_POST["only_errors"])echo"<p class='message' title='".h($f->info)."'>".sprintf('åŸ·è¡ŒæŸ¥è©¢ OKï¼Œ%d è¡Œå—å½±éŸ¿ã€‚',$ta)."$li\n";}echo($kj?"<div id='$lj' class='hidden'>\n$kj</div>\n":"");if($Jc){echo"<div id='$Kc' class='hidden explain'>\n";select($Jc,$g,$Gf);echo"</div>\n";}}$Kh=microtime(true);}while($f->next_result());}$G=substr($G,$C);$C=0;}}}}if($uc)echo"<p class='message'>".'æ²’æœ‰å‘½ä»¤å¯åŸ·è¡Œã€‚'."\n";elseif($_POST["only_errors"]){echo"<p class='message'>".sprintf('å·²é †åˆ©åŸ·è¡Œ %d å€‹æŸ¥è©¢ã€‚',$ob-count($Bc))," <span class='time'>(".format_time($wi).")</span>\n";}elseif($Bc&&$ob>1)echo"<p class='error'>".'æŸ¥è©¢ç™¼ç”ŸéŒ¯èª¤'.": ".implode("",$Bc)."\n";}else
echo"<p class='error'>".upload_error($G)."\n";}echo'
<form action="" method="post" enctype="multipart/form-data" id="form">
';$Hc="<input type='submit' value='".'åŸ·è¡Œ'."' title='Ctrl+Enter'>";if(!isset($_GET["import"])){$xg=$_GET["sql"];if($_POST)$xg=$_POST["query"];elseif($_GET["history"]=="all")$xg=$Dd;elseif($_GET["history"]!="")$xg=$Dd[$_GET["history"]][0];echo"<p>";textarea("query",$xg,20);echo
script(($_POST?"":"qs('textarea').focus();\n")."qs('#form').onsubmit = partial(sqlSubmit, qs('#form'), '".js_escape(remove_from_uri("sql|limit|error_stops|only_errors|history"))."');"),"<p>$Hc\n",'é™åˆ¶è¡Œæ•¸'.": <input type='number' name='limit' class='size' value='".h($_POST?$_POST["limit"]:$_GET["limit"])."'>\n";}else{echo"<fieldset><legend>".'æª”æ¡ˆä¸Šå‚³'."</legend><div>";$wd=(extension_loaded("zlib")?"[.gz]":"");echo(ini_bool("file_uploads")?"SQL$wd (&lt; ".ini_get("upload_max_filesize")."B): <input type='file' name='sql_file[]' multiple>\n$Hc":'æª”æ¡ˆä¸Šå‚³å·²ç¶“è¢«åœç”¨ã€‚'),"</div></fieldset>\n";$Ld=$b->importServerPath();if($Ld){echo"<fieldset><legend>".'å¾ä¼ºæœå™¨'."</legend><div>",sprintf('ç¶²é ä¼ºæœå™¨æª”æ¡ˆ %s',"<code>".h($Ld)."$wd</code>"),' <input type="submit" name="webfile" value="'.'åŸ·è¡Œæª”æ¡ˆ'.'">',"</div></fieldset>\n";}echo"<p>";}echo
checkbox("error_stops",1,($_POST?$_POST["error_stops"]:isset($_GET["import"])||$_GET["error_stops"]),'å‡ºéŒ¯æ™‚åœæ­¢')."\n",checkbox("only_errors",1,($_POST?$_POST["only_errors"]:isset($_GET["import"])||$_GET["only_errors"]),'åƒ…é¡¯ç¤ºéŒ¯èª¤è¨Šæ¯')."\n","<input type='hidden' name='token' value='$T'>\n";if(!isset($_GET["import"])&&$Dd){print_fieldset("history",'ç´€éŒ„',$_GET["history"]!="");for($X=end($Dd);$X;$X=prev($Dd)){$y=key($Dd);list($xg,$li,$pc)=$X;echo'<a href="'.h(ME."sql=&history=$y").'">'.'ç·¨è¼¯'."</a>"." <span class='time' title='".@date('Y-m-d',$li)."'>".@date("H:i:s",$li)."</span>"." <code class='jush-".JUSH."'>".shorten_utf8(ltrim(str_replace("\n"," ",str_replace("\r","",preg_replace('~^(#|-- ).*~m','',$xg)))),80,"</code>").($pc?" <span class='time'>($pc)</span>":"")."<br>\n";}echo"<input type='submit' name='clear' value='".'æ¸…é™¤'."'>\n","<a href='".h(ME."sql=&history=all")."'>".'ç·¨è¼¯å…¨éƒ¨'."</a>\n","</div></fieldset>\n";}echo'</form>
...
',checkbox("defaults",1,($_POST?$_POST["defaults"]:adminer_setting("defaults")),'é è¨­å€¼',"columnShow(this.checked, 5)","jsonly");$rb=($_POST?$_POST["comments"]:adminer_setting("comments"));echo(support("comment")?checkbox("comments",1,$rb,'è¨»è§£',"editingCommentsClick(this, true);","jsonly").' '.(preg_match('~\n~',$J["Comment"])?"<textarea name='Comment' rows='2' cols='20'".($rb?"":" class='hidden'").">".h($J["Comment"])."</textarea>":'<input name="Comment" value="'.h($J["Comment"]).'" data-maxlength="'.(min_version(5.5)?2048:60).'"'.($rb?"":" class='hidden'").'>'):''),'<p>
<input type="submit" value="å„²å­˜">
';}echo'
';if($a!=""){echo'<input type="submit" name="drop" value="åˆªé™¤">',confirm(sprintf('åˆªé™¤ %s?',$a));}if(support("partitioning")){$Yf=preg_match('~RANGE|LIST~',$J["partition_by"]);print_fieldset("partition",'åˆ†å€é¡å‹',$J["partition_by"]);echo'<p>
...
åˆ†å€: <input type="number" name="partitions" class="size',($Yf||!$J["partition_by"]?" hidden":""),'" value="',h($J["partitions"]),'">
...
<th><input type="submit" class="wayoff">','æ¬„ä½'.($xe?"<span class='idxopts".($uh?"":" hidden")."'> (".'é•·åº¦'.")</span>":"");if($xe||support("descidx"))echo
checkbox("options",1,$uh,'é¸é …',"indexOptionsShow(this.checked)","jsonly")."\n";echo'<th id="label-name">åç¨±
<th><noscript>',"<input type='image' class='icon' name='add[0]' src='".h(preg_replace("~\\?.*~","",ME)."?file=plus.gif&version=5.0.4")."' alt='+' title='".'æ–°å¢ä¸‹ä¸€ç­†'."'>",'</noscript>
</thead>
';if($F){echo"<tr><td>PRIMARY<td>";foreach($F["columns"]as$y=>$d){echo
select_input(" disabled",$o,$d),"<label><input disabled type='checkbox'>".'é™å†ª (éæ¸›)'."</label> ";}echo"<td><td>\n";}$ge=1;foreach($J["indexes"]as$w){if(!$_POST["drop_col"]||$ge!=key($_POST["drop_col"])){echo"<tr><td>".html_select("indexes[$ge][type]",array(-1=>"")+$Pd,$w["type"],($ge==count($J["indexes"])?"indexesAddRow.call(this);":""),"label-type"),"<td>";ksort($w["columns"]);$t=1;foreach($w["columns"]as$y=>$d){echo"<span>".select_input(" name='indexes[$ge][columns][$t]' title='".'æ¬„ä½'."'",($o?array_combine($o,$o):$o),$d,"partial(".($t==count($w["columns"])?"indexesAddColumn":"indexesChangeColumn").", '".js_escape(JUSH=="sql"?"":$_GET["indexes"]."_")."')"),"<span class='idxopts".($uh?"":" hidden")."'>",($xe?"<input type='number' name='indexes[$ge][lengths][$t]' class='size' value='".h($w["lengths"][$y])."' title='".'é•·åº¦'."'>":""),(support("descidx")?checkbox("indexes[$ge][descs][$t]",1,$w["descs"][$y],'é™å†ª (éæ¸›)'):""),"</span> </span>";$t++;}echo"<td><input name='indexes[$ge][name]' value='".h($w["name"])."' autocapitalize='off' aria-labelledby='label-name'>\n","<td><input type='image' class='icon' name='drop_col[$ge]' src='".h(preg_replace("~\\?.*~","",ME)."?file=cross.gif&version=5.0.4")."' alt='x' title='".'ç§»é™¤'."'>".script("qsl('input').onclick = partial(editingRemoveRow, 'indexes\$1[type]');");}$ge++;}echo'</table>
</div>
<p>
<input type="submit" value="å„²å­˜">
<input type="hidden" name="token" value="',$T,'">
...
as$j){if(count($i)==1||$j!=""){if(!create_database($j,$J["collation"]))$Ph=false;$qe=$j;}}restart_session();set_session("dbs",null);queries_redirect(ME."db=".urlencode($qe),'å·²å»ºç«‹è³‡æ–™åº«ã€‚',$Ph);}}else{if(!$J["collation"])redirect(substr(ME,0,-1));query_redirect("ALTER DATABASE ".idf_escape($B).(preg_match('~^[a-z0-9_]+$~i',$J["collation"])?" COLLATE $J[collation]":""),substr(ME,0,-1),'å·²ä¿®æ”¹è³‡æ–™åº«ã€‚');}}page_header(DB!=""?'ä¿®æ”¹è³‡æ–™åº«':'å»ºç«‹è³‡æ–™åº«',$m,array(),h(DB));$jb=collations();$B=DB;if($_POST)$B=$J["name"];elseif(DB!="")$J["collation"]=db_collation(DB,$jb);elseif(JUSH=="sql"){foreach(get_vals("SHOW GRANTS")as$qd){if(preg_match('~ ON (`(([^\\\\`]|``|\\\\.)*)%`\.\*)?~',$qd,$A)&&$A[1]){$B=stripcslashes(idf_unescape("`$A[2]`"));break;}}}echo'
<form action="" method="post">
<p>
',($_POST["add_x"]||strpos($B,"\n")?'<textarea autofocus name="name" rows="10" cols="40">'.h($B).'</textarea><br>':'<input name="name" autofocus value="'.h($B).'" data-maxlength="64" autocapitalize="off">')."\n".($jb?html_select("collation",array(""=>"(".'æ ¡å°'.")")+$jb,$J["collation"]).doc_link(array('sql'=>"charset-charsets.html",'mariadb'=>"supported-character-sets-and-collations/",'mssql'=>"relational-databases/system-functions/sys-fn-helpcollations-transact-sql",)):""),'<input type="submit" value="å„²å­˜">
';if(DB!="")echo"<input type='submit' name='drop' value='".'åˆªé™¤'."'>".confirm(sprintf('åˆªé™¤ %s?',DB))."\n";elseif(!$_POST["add_x"]&&$_GET["db"]=="")echo"<input type='image' class='icon' name='add' src='".h(preg_replace("~\\?.*~","",ME)."?file=plus.gif&version=5.0.4")."' alt='+' title='".'æ–°å¢ä¸‹ä¸€ç­†'."'>\n";echo'<input type="hidden" name="token" value="',$T,'">
</form>
...
';}elseif(isset($_GET["foreign"])){$a=$_GET["foreign"];$B=$_GET["name"];$J=$_POST;if($_POST&&!$m&&!$_POST["add"]&&!$_POST["change"]&&!$_POST["change-js"]){if(!$_POST["drop"]){$J["source"]=array_filter($J["source"],'strlen');ksort($J["source"]);$ei=array();foreach($J["source"]as$y=>$X)$ei[$y]=$J["target"][$y];$J["target"]=$ei;}if(JUSH=="sqlite")$H=recreate_table($a,$a,array(),array(),array(" $B"=>($J["drop"]?"":" ".format_foreign_key($J))));else{$c="ALTER TABLE ".table($a);$H=($B==""||queries("$c DROP ".(JUSH=="sql"?"FOREIGN KEY ":"CONSTRAINT ").idf_escape($B)));if(!$J["drop"])$H=queries("$c ADD".format_foreign_key($J));}queries_redirect(ME."table=".urlencode($a),($J["drop"]?'å·²åˆªé™¤å¤–ä¾†éµã€‚':($B!=""?'å·²ä¿®æ”¹å¤–ä¾†éµã€‚':'å·²å»ºç«‹å¤–ä¾†éµã€‚')),$H);if(!$J["drop"])$m="$m<br>".'ä¾†æºåˆ—å’Œç›®æ¨™åˆ—å¿…é ˆå…·æœ‰ç›¸åŒçš„è³‡æ–™é¡å‹ï¼Œåœ¨ç›®æ¨™åˆ—ä¸Šå¿…é ˆæœ‰ä¸€å€‹ç´¢å¼•ä¸¦ä¸”å¼•ç”¨çš„è³‡æ–™å¿…é ˆå­˜åœ¨ã€‚';}page_header('å¤–ä¾†éµ',$m,array("table"=>$a),h($a));if($_POST){ksort($J["source"]);if($_POST["add"])$J["source"][]="";elseif($_POST["change"]||$_POST["change-js"])$J["target"]=array();}elseif($B!=""){$hd=foreign_keys($a);$J=$hd[$B];$J["source"][]="";}else{$J["table"]=$a;$J["source"]=array("");}echo'
<form action="" method="post">
';$Bh=array_keys(fields($a));if($J["db"]!="")$f->select_db($J["db"]);if($J["ns"]!=""){$Kf=get_schema();set_schema($J["ns"]);}$Eg=array_keys(array_filter(table_status('',true),'Adminer\fk_support'));$ei=array_keys(fields(in_array($J["table"],$Eg)?$J["table"]:reset($Eg)));$uf="this.form['change-js'].value = '1'; this.form.submit();";echo"<p>".'ç›®æ¨™è³‡æ–™è¡¨'.": ".html_select("table",$Eg,$J["table"],$uf)."\n";if(support("scheme")){$ch=array_filter($b->schemas(),function($bh){return!preg_match('~^information_schema$~i',$bh);});echo'è³‡æ–™è¡¨çµæ§‹'.": ".html_select("ns",$ch,$J["ns"]!=""?$J["ns"]:$_GET["ns"],$uf);if($J["ns"]!="")set_schema($Kf);}elseif(JUSH!="sqlite"){$Qb=array();foreach($b->databases()as$j){if(!information_schema($j))$Qb[]=$j;}echo'è³‡æ–™åº«'.": ".html_select("db",$Qb,$J["db"]!=""?$J["db"]:$_GET["db"],$uf);}echo'<input type="hidden" name="change-js" value="">
...
';}elseif(isset($_GET["view"])){$a=$_GET["view"];$J=$_POST;$Lf="VIEW";if(JUSH=="pgsql"&&$a!=""){$O=table_status($a);$Lf=strtoupper($O["Engine"]);}if($_POST&&!$m){$B=trim($J["name"]);$za=" AS\n$J[select]";$_e=ME."table=".urlencode($B);$Oe='å·²ä¿®æ”¹æª¢è¦–è¡¨ã€‚';$U=($_POST["materialized"]?"MATERIALIZED VIEW":"VIEW");if(!$_POST["drop"]&&$a==$B&&JUSH!="sqlite"&&$U=="VIEW"&&$Lf=="VIEW")query_redirect((JUSH=="mssql"?"ALTER":"CREATE OR REPLACE")." VIEW ".table($B).$za,$_e,$Oe);else{$gi=$B."_adminer_".uniqid();drop_create("DROP $Lf ".table($a),"CREATE $U ".table($B).$za,"DROP $U ".table($B),"CREATE $U ".table($gi).$za,"DROP $U ".table($gi),($_POST["drop"]?substr(ME,0,-1):$_e),'å·²åˆªé™¤æª¢è¦–è¡¨ã€‚',$Oe,'å·²å»ºç«‹æª¢è¦–è¡¨ã€‚',$a,$B);}}if(!$_POST&&$a!=""){$J=view($a);$J["name"]=$a;$J["materialized"]=($Lf!="VIEW");if(!$m)$m=error();}page_header(($a!=""?'ä¿®æ”¹æª¢è¦–è¡¨':'å»ºç«‹æª¢è¦–è¡¨'),$m,array("table"=>$a),h($a));echo'
<form action="" method="post">
...
';}elseif(isset($_GET["event"])){$aa=$_GET["event"];$Xd=array("YEAR","QUARTER","MONTH","DAY","HOUR","MINUTE","WEEK","SECOND","YEAR_MONTH","DAY_HOUR","DAY_MINUTE","DAY_SECOND","HOUR_MINUTE","HOUR_SECOND","MINUTE_SECOND");$Lh=array("ENABLED"=>"ENABLE","DISABLED"=>"DISABLE","SLAVESIDE_DISABLED"=>"DISABLE ON SLAVE");$J=$_POST;if($_POST&&!$m){if($_POST["drop"])query_redirect("DROP EVENT ".idf_escape($aa),substr(ME,0,-1),'å·²åˆªé™¤äº‹ä»¶ã€‚');elseif(in_array($J["INTERVAL_FIELD"],$Xd)&&isset($Lh[$J["STATUS"]])){$ah="\nON SCHEDULE ".($J["INTERVAL_VALUE"]?"EVERY ".q($J["INTERVAL_VALUE"])." $J[INTERVAL_FIELD]".($J["STARTS"]?" STARTS ".q($J["STARTS"]):"").($J["ENDS"]?" ENDS ".q($J["ENDS"]):""):"AT ".q($J["STARTS"]))." ON COMPLETION".($J["ON_COMPLETION"]?"":" NOT")." PRESERVE";queries_redirect(substr(ME,0,-1),($aa!=""?'å·²ä¿®æ”¹äº‹ä»¶ã€‚':'å·²å»ºç«‹äº‹ä»¶ã€‚'),queries(($aa!=""?"ALTER EVENT ".idf_escape($aa).$ah.($aa!=$J["EVENT_NAME"]?"\nRENAME TO ".idf_escape($J["EVENT_NAME"]):""):"CREATE EVENT ".idf_escape($J["EVENT_NAME"]).$ah)."\n".$Lh[$J["STATUS"]]." COMMENT ".q($J["EVENT_COMMENT"]).rtrim(" DO\n$J[EVENT_DEFINITION]",";").";"));}}page_header(($aa!=""?'ä¿®æ”¹äº‹ä»¶'.": ".h($aa):'å»ºç«‹äº‹ä»¶'),$m);if(!$J&&$aa!=""){$K=get_rows("SELECT * FROM information_schema.EVENTS WHERE EVENT_SCHEMA = ".q(DB)." AND EVENT_NAME = ".q($aa));$J=reset($K);}echo'
<form action="" method="post">
<table class="layout">
...
<tr><th>æ¯<td><input type="number" name="INTERVAL_VALUE" value="',h($J["INTERVAL_VALUE"]),'" class="size"> ',html_select("INTERVAL_FIELD",$Xd,$J["INTERVAL_FIELD"]),'<tr><th>ç‹€æ…‹<td>',html_select("STATUS",$Lh,$J["STATUS"]),'<tr><th>è¨»è§£<td><input name="EVENT_COMMENT" value="',h($J["EVENT_COMMENT"]),'" data-maxlength="64">
...
';}elseif(isset($_GET["procedure"])){$da=($_GET["name"]?:$_GET["procedure"]);$Vg=(isset($_GET["function"])?"FUNCTION":"PROCEDURE");$J=$_POST;$J["fields"]=(array)$J["fields"];if($_POST&&!process_fields($J["fields"])&&!$m){$Hf=routine($_GET["procedure"],$Vg);$gi="$J[name]_adminer_".uniqid();drop_create("DROP $Vg ".routine_id($da,$Hf),create_routine($Vg,$J),"DROP $Vg ".routine_id($J["name"],$J),create_routine($Vg,array("name"=>$gi)+$J),"DROP $Vg ".routine_id($gi,$J),substr(ME,0,-1),'å·²åˆªé™¤ç¨‹åºã€‚','å·²ä¿®æ”¹å­ç¨‹åºã€‚','å·²å»ºç«‹å­ç¨‹åºã€‚',$da,$J["name"]);}page_header(($da!=""?(isset($_GET["function"])?'ä¿®æ”¹å‡½å¼':'ä¿®æ”¹é å­˜ç¨‹åº').": ".h($da):(isset($_GET["function"])?'å»ºç«‹å‡½å¼':'å»ºç«‹é å­˜ç¨‹åº')),$m);if(!$_POST&&$da!=""){$J=routine($_GET["procedure"],$Vg);$J["name"]=$da;}$jb=get_vals("SHOW CHARACTER SET");sort($jb);$Wg=routine_languages();echo($jb?"<datalist id='collations'>".optionlist($jb)."</datalist>":""),'
<form action="" method="post" id="form">
<p>åç¨±: <input name="name" value="',h($J["name"]),'" data-maxlength="64" autocapitalize="off">
',($Wg?'èªè¨€'.": ".html_select("language",$Wg,$J["language"])."\n":""),'<input type="submit" value="å„²å­˜">
<div class="scrollable">
<table class="nowrap">
';edit_fields($J["fields"],$jb,$Vg);if(isset($_GET["function"])){echo"<tr><td>".'å›å‚³é¡å‹';edit_type("returns",$J["returns"],$jb,array(),(JUSH=="pgsql"?array("void","trigger"):array()));}echo'</table>
',script("editFields();"),'</div>
...
';}elseif(isset($_GET["trigger"])){$a=$_GET["trigger"];$B=$_GET["name"];$Ei=trigger_options();$J=(array)trigger($B,$a)+array("Trigger"=>$a."_bi");if($_POST){if(!$m&&in_array($_POST["Timing"],$Ei["Timing"])&&in_array($_POST["Event"],$Ei["Event"])&&in_array($_POST["Type"],$Ei["Type"])){$rf=" ON ".table($a);$fc="DROP TRIGGER ".idf_escape($B).(JUSH=="pgsql"?$rf:"");$_e=ME."table=".urlencode($a);if($_POST["drop"])query_redirect($fc,$_e,'å·²åˆªé™¤è§¸ç™¼å™¨ã€‚');else{if($B!="")queries($fc);queries_redirect($_e,($B!=""?'å·²ä¿®æ”¹è§¸ç™¼å™¨ã€‚':'å·²å»ºç«‹è§¸ç™¼å™¨ã€‚'),queries(create_trigger($rf,$_POST)));if($B!="")queries(create_trigger($rf,$J+array("Type"=>reset($Ei["Type"]))));}}$J=$_POST;}page_header(($B!=""?'ä¿®æ”¹è§¸ç™¼å™¨'.": ".h($B):'å»ºç«‹è§¸ç™¼å™¨'),$m,array("table"=>$a));echo'
<form action="" method="post" id="form">
<table class="layout">
<tr><th>æ™‚é–“<td>',html_select("Timing",$Ei["Timing"],$J["Timing"],"triggerChange(/^".preg_quote($a,"/")."_[ba][iud]$/, '".js_escape($a)."', this.form);"),'<tr><th>äº‹ä»¶<td>',html_select("Event",$Ei["Event"],$J["Event"],"this.form['Timing'].onchange();"),(in_array("UPDATE OF",$Ei["Event"])?" <input name='Of' value='".h($J["Of"])."' class='hidden'>":""),'<tr><th>é¡å‹<td>',html_select("Type",$Ei["Type"],$J["Type"]),'</table>
...
as$jf=>$Sg){if(preg_match('~^(.+)(\(.*\))?$~U',$jf,$A))grant("REVOKE",array_keys($Sg),$A[2]," ON $A[1] FROM $df");}}}queries_redirect(ME."privileges=",(isset($_GET["host"])?'å·²ä¿®æ”¹ä½¿ç”¨è€…ã€‚':'å·²å»ºç«‹ä½¿ç”¨è€…ã€‚'),!$m);if($Eb)$f->query("DROP USER $df");}}page_header((isset($_GET["host"])?'å¸³è™Ÿ'.": ".h("$ha@$_GET[host]"):'å»ºç«‹ä½¿ç”¨è€…'),$m,array("privileges"=>array('','æ¬Šé™')));$J=$_POST;if($J)$rd=$bf;else{$J=$_GET+array("host"=>get_val("SELECT SUBSTRING_INDEX(CURRENT_USER, '@', -1)"));$J["pass"]=$pf;if($pf!="")$J["hashed"]=true;$rd[(DB==""||$rd?"":idf_escape(addcslashes(DB,"%_\\"))).".*"]=array();}echo'<form action="" method="post">
<table class="layout">
...
';echo"<table class='odds'>\n","<thead><tr><th colspan='2'>".'æ¬Šé™'.doc_link(array('sql'=>"grant.html#priv_level"));$t=0;foreach($rd
as$jf=>$qd){echo'<th>'.($jf!="*.*"?"<input name='objects[$t]' value='".h($jf)."' size='10' autocapitalize='off'>":"<input type='hidden' name='objects[$t]' value='*.*' size='10'>*.*");$t++;}echo"</thead>\n";foreach(array(""=>"","Server Admin"=>'ä¼ºæœå™¨',"Databases"=>'è³‡æ–™åº«',"Tables"=>'è³‡æ–™è¡¨',"Columns"=>'æ¬„ä½',"Procedures"=>'ç¨‹åº',)as$zb=>$Wb){foreach((array)$tg[$zb]as$sg=>$pb){echo"<tr><td".($Wb?">$Wb<td":" colspan='2'").' lang="en" title="'.h($pb).'">'.h($sg);$t=0;foreach($rd
...
';}elseif(isset($_GET["processlist"])){if(support("kill")){if($_POST&&!$m){$me=0;foreach((array)$_POST["kill"]as$X){if(kill_process($X))$me++;}queries_redirect(ME."processlist=",sprintf('%d å€‹ Process(es) è¢«çµ‚æ­¢ã€‚',$me),$me||!$_POST["kill"]);}}page_header('è™•ç†ç¨‹åºåˆ—è¡¨',$m);echo'
<form action="" method="post">
<div class="scrollable">
...
as$ie=>$W)$_.=where_link($t++,$ie,$W);}$X=select_value($X,$_,$n,$ki);$u=h("val[$Ni][".bracket_escape($y)."]");$Y=$_POST["val"][$Ni][bracket_escape($y)];$oc=!is_array($J[$y])&&is_utf8($X)&&$K[$Xe][$y]==$J[$y]&&!$nd[$y]&&!$n["generated"];$ii=preg_match('~text|lob~',$n["type"]);echo"<td id='$u'";if(($_GET["modify"]&&$oc)||$Y!==null){$xd=h($Y!==null?$Y:$J[$y]);echo">".($ii?"<textarea name='$u' cols='30' rows='".(substr_count($J[$y],"\n")+1)."'>$xd</textarea>":"<input name='$u' value='$xd' size='$xe[$y]'>");}else{$Be=strpos($X,"<i>â€¦</i>");echo" data-text='".($Be?2:($ii?1:0))."'".($oc?"":" data-warning='".h('ä½¿ç”¨ç·¨è¼¯é€£çµä¾†ä¿®æ”¹ã€‚')."'").">$X";}}}if($Ha)echo"<td>";$b->backwardKeysPrint($Ha,$K[$Xe]);echo"</tr>\n";}if(is_ajax())exit;echo"</table>\n","</div>\n";}if(!is_ajax()){if($K||$D){$Gc=true;if($_GET["page"]!="last"){if($z==""||(count($K)<$z&&($K||!$D)))$kd=($D?$D*$z:0)+count($K);elseif(JUSH!="sql"||!$be){$kd=($be?false:found_rows($R,$Z));if($kd<max(1e4,2*($D+1)*$z))$kd=reset(slow_query(count_rows($a,$Z,$be,$sd)));else$Gc=false;}}$Sf=($z!=""&&($kd===false||$kd>$z||$D));if($Sf){echo(($kd===false?count($K)+1:$kd-$D*$z)>$z?'<p><a href="'.h(remove_from_uri("page")."&page=".($D+1)).'" class="loadmore">'.'è¼‰å…¥æ›´å¤šè³‡æ–™'.'</a>'.script("qsl('a').onclick = partial(selectLoadMore, ".(+$z).", '".'è¼‰å…¥ä¸­'."â€¦');",""):''),"\n";}}echo"<div class='footer'><div>\n";if($K||$D){if($Sf){$Ie=($kd===false?$D+(count($K)>=$z?2:1):floor(($kd-1)/$z));echo"<fieldset>";if(JUSH!="simpledb"){echo"<legend><a href='".h(remove_from_uri("page"))."'>".'é '."</a></legend>",script("qsl('a').onclick = function () { pageClick(this.href, +prompt('".'é '."', '".($D+1)."')); return false; };"),pagination(0,$D).($D>5?" â€¦":"");for($t=max(1,$D-4);$t<min($Ie,$D+5);$t++)echo
pagination($t,$D);if($Ie>0){echo($D+5<$Ie?" â€¦":""),($Gc&&$kd!==false?pagination($Ie,$D):" <a href='".h(remove_from_uri("page")."&page=last")."' title='~$Ie'>".'æœ€å¾Œä¸€é '."</a>");}}else{echo"<legend>".'é '."</legend>",pagination(0,$D).($D>1?" â€¦":""),($D?pagination($D,$D):""),($Ie>$D?pagination($D+1,$D).($Ie>$D+1?" â€¦":""):"");}echo"</fieldset>\n";}echo"<fieldset>","<legend>".'æ‰€æœ‰çµæœ'."</legend>";$cc=($Gc?"":"~ ").$kd;$vf="var checked = formChecked(this, /check/); selectCount('selected', this.checked ? '$cc' : checked); selectCount('selected2', this.checked || !checked ? '$cc' : checked);";echo
checkbox("all",1,0,($kd!==false?($Gc?"":"~ ").sprintf('%d è¡Œ',$kd):""),$vf)."\n","</fieldset>\n";if($b->selectCommandPrint()){echo'<fieldset',($_GET["modify"]?'':' class="jsonly"'),'><legend>ä¿®æ”¹</legend><div>
...
';}$id=$b->dumpFormat();foreach((array)$_GET["columns"]as$d){if($d["fun"]){unset($id['sql']);break;}}if($id){print_fieldset("export",'åŒ¯å‡º'." <span id='selected2'></span>");$Pf=$b->dumpOutput();echo($Pf?html_select("output",$Pf,$sa["output"])." ":""),html_select("format",$id,$sa["format"])," <input type='submit' name='export' value='".'åŒ¯å‡º'."'>\n","</div></fieldset>\n";}$b->selectEmailPrint(array_filter($tc,'strlen'),$e);}echo"</div></div>\n";if($b->selectImportPrint()){echo"<div>","<a href='#import'>".'åŒ¯å…¥'."</a>",script("qsl('a').onclick = partial(toggle, 'import');",""),"<span id='import'".($_POST["import"]?"":" class='hidden'").">: ","<input type='file' name='csv_file'> ",html_select("separator",array("csv"=>"CSV,","csv;"=>"CSV;","tsv"=>"TSV"),$sa["format"])," <input type='submit' name='import' value='".'åŒ¯å…¥'."'>","</span>","</div>";}echo"<input type='hidden' name='token' value='$T'>\n","</form>\n",(!$sd&&$L?"":script("tableCheck();"));}}}if(is_ajax()){ob_end_clean();exit;}}elseif(isset($_GET["variables"])){$O=isset($_GET["status"]);page_header($O?'ç‹€æ…‹':'è®Šæ•¸');$dj=($O?show_status():show_variables());if(!$dj)echo"<p class='message'>".'æ²’æœ‰è³‡æ–™è¡Œã€‚'."\n";else{echo"<table>\n";foreach($dj
